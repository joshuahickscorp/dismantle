    ...
    def execute(
        ...
        *,
        ...
        try:
            ...
            observations: List[Dict[str, Any]] = []
            # An evidence-complete resident lane must enter the closed-turn
            # budget path on its first call.  HCLI_NO_TOOLS used to suppress
            # the catalog text but left this flag false, so the initial call
            # could still consume the whole worker packet before the reducer
            # was allowed to protect the source snapshot.
            ...
            for round_index in range(self.MAX_TOOL_ROUNDS):
                ...
                # success only replays the same confused plan. Close the
                # catalog before the next model call so a local resident
                # cannot spend several minutes on a mixed success/failure
                # loop. One final no-tools call still gives it a chance to
                # emit the mutation/answer it can justify from disk evidence.
