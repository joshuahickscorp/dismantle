...
"""Activation-aware packing program for GLM-5.2.

Campaign ordering, frozen by the real-activation pilot:

    measurement shardwise  ->  allocation global  ->  packing shardwise

The measured finding this implements (see GLM52_REBUILD_PILOT_RECEIPT.json,
GLM52_REAL_ACTIVATION_SWEEP.json, GLM52_ACTIVATION_AWARE_REPLICATION.json):

  * constant-mean null on REAL activations is ~0.65 (Gaussian proxies read ~0.13)
  * activation-aware projection: cosine 0.755 at 0.167 BPW, 0.793 at 0.667 BPW
  * raw-weight low-rank at 0.667 BPW sits BELOW the null (0.384)
  * replicated 12/12 routed experts; raw-weight 0/12
  * top 16 activation directions carry the bulk of variance
  * neighbouring-layer bases transfer; every tensor records which basis it used
...
Non-negotiable:
  ...
  * Reconstruction error may be logged; it does NOT drive allocation.
    Allocation maximises functional preservation against the real-activation null.
  * Do not touch the sealed Math-Preserve artifact, odyssey/launch/, or any
    Codex-owned file.

    .venv/glm52/bin/python tools/condense/glm52_activation_aware_pack.py --dry-run
    .venv/glm52/bin/python tools/condense/glm52_activation_aware_pack.py --full \\
        --target-bpw 49/50 --shards 1-282

Self-test (no parent bodies required):

    python3.12 tools/condense/glm52_activation_aware_pack.py selftest
"""
...
if str(REPO / "tools" / "foundry") not in sys.path:
    sys.path.insert(0, str(REPO / "tools" / "foundry"))

# ---------------------------------------------------------------------------
# Sealed campaign constants
# ---------------------------------------------------------------------------
SCHEMA = "hawking.glm52.activation_aware_pack.v1"
MAGIC = b"GLM52AAP"
HEADER_BYTES = 64
# Same whole-model denominator the Math-Preserve receipt used.
ORIGINAL_WEIGHT_COUNT = 753_329_940_480
...
def assert_disk_floor(extra_bytes: int = 0, *, path: Path | None = None,
                      floor: int = DISK_FLOOR_BYTES) -> int:
...
# ---------------------------------------------------------------------------
# Exact complete-byte accounting (same slots as the Math-Preserve receipt)
# ---------------------------------------------------------------------------
...
class ByteLedger:
    ...
    def as_dict(self, weight_count: int = ORIGINAL_WEIGHT_COUNT) -> dict[str, Any]:
        ...
        return {
            ...
            "scope": "whole_model" if weight_count == ORIGINAL_WEIGHT_COUNT else "subset",
            "component_bytes": dict(self.components),
            "total_bytes": total,
            "itemization_reconciles": True,
            "complete_bits": str(bits),
            "itemized_bits": str(bits),
            "reserve_bits": "0",
            "original_weight_count": int(weight_count),
            "complete_bpw_exact": f"{bpw.numerator}/{bpw.denominator}",
            "complete_bpw_float": float(bpw),
            "note": (
                "Activation-aware pack: every byte counted, including bases "
                "(runtime_tables), coefficients (codebooks), headers (metadata), "
                "pass-through tensors, and packaging. No exclusions."
            ),
        }
...
def parse_bpw(value: str | float | Fraction) -> Fraction:
    ...
    return Fraction(text).limit_denominator(10_000_000)


# ---------------------------------------------------------------------------
# Safetensors streaming reader (one tensor at a time; never load a whole shard)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Teacher-capsule basis provenance
# ---------------------------------------------------------------------------
...
class BasisProvenance:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            "tensor_layer": self.tensor_layer,
            "basis_layer": self.basis_layer,
            "capsule_file": self.capsule_file,
            "capsule_key": self.capsule_key,
            "hidden": self.hidden,
            "n_activation_rows": self.n_activation_rows,
            "source": self.source,
        }
...
def discover_capsule_layers(capsule_dir: Path = CAPSULE_DIR) -> dict[int, tuple[Path, str]]:
    ...
    if not capsule_dir.is_dir():
        return out
    key_re = re.compile(r"layer_(\d+)/pre_router_hidden$")
            ...
            with np.load(path) as z:
                for key in z.files:
                    ...
                    if not m:
                        continue
                    layer = int(m.group(1))
                    # Prefer a single-layer capsule when both exist.
                    if layer in out and out[layer][0].stem.count("L") == 2:
                        # existing is multi; single-layer names look like L10_L10
...
def nearest_basis_layer(layer: int, available: Sequence[int]) -> int:
    ...
    for cand in ordered[1:]:
        d = abs(cand - layer)
        # Prefer lower index on a tie so the choice is deterministic.
        if d < best_d or (d == best_d and cand < best):
            best, best_d = cand, d
    return best


def resolve_basis_for_tensor(
    name: str,
    capsule_map: dict[int, tuple[Path, str]],
) -> BasisProvenance:
    tensor_layer = layer_of(name)
    available = sorted(capsule_map)
    if tensor_layer is None:
        # Embeddings sit before layer 0; lm_head after the last layer. Use the
        # nearest endpoint capsule and record the choice.
        prefer = 0 if name.startswith("model.embed") else (N_LAYERS - 1)
        basis_layer = nearest_basis_layer(prefer, available)
    else:
        basis_layer = (
            tensor_layer if tensor_layer in capsule_map
            else nearest_basis_layer(tensor_layer, available)
        )
    path, key = capsule_map[basis_layer]
    # Row count is read lazily when the basis is actually built; provisional 0 here
    # is overwritten by build_basis. Keep the field so the schema is stable.
...
# ---------------------------------------------------------------------------
# Activation basis + functional scoring
# ---------------------------------------------------------------------------
@dataclass
class ActivationBasis:
    ...
    layer: int
    basis: np.ndarray          # [H, max_rank] float32, orthonormal columns
    singular_values: np.ndarray
    variance_frac: list[float]  # cumulative variance explained at rank r (1-indexed via index r-1)
    provenance: BasisProvenance
    X_hold: np.ndarray         # held-out activations for scoring [N_hold, H]
    X_fit_mean: np.ndarray     # for diagnostics only
...
def build_basis(
    ...
    *,
    ...
    if X.shape[1] != HIDDEN:
        # Still usable; record the actual width.
        hidden = int(X.shape[1])
    else:
        hidden = HIDDEN
    n = X.shape[0]
    rng = np.random.default_rng(seed ^ (provenance.basis_layer * 0x9E3779B1))
    perm = rng.permutation(n)
    n_hold = max(HOLDOUT_MIN, int(round(n * HELD_OUT_FRAC)))
    n_hold = min(n_hold, n // 5 if n >= 5 else max(1, n // 5))
    # Keep at least half the rows for the fit when the capsule is small.
    ...
    Xc = X_fit - mu
    # Economy SVD of the (centered) activation matrix. Columns of V are principal
    # directions in activation space -- the structure raw-weight SVD never sees.
    _u, s, vt = np.linalg.svd(Xc, full_matrices=False)
    max_rank = min(int(max_rank), vt.shape[0], hidden)
    basis = vt[:max_rank].T.astype(np.float32, copy=True)  # [H, max_rank]
    energy = (s.astype(np.float64) ** 2)
    total = float(energy.sum()) + 1e-30
    cum = np.cumsum(energy[:max_rank]) / total
    prov = BasisProvenance(
        tensor_layer=provenance.tensor_layer,
        basis_layer=provenance.basis_layer,
        capsule_file=path.name,
        capsule_key=key,
        hidden=hidden,
        n_activation_rows=int(n),
    )
    return ActivationBasis(
        layer=provenance.basis_layer,
        basis=basis,
        singular_values=s[:max_rank].astype(np.float32),
        variance_frac=[float(x) for x in cum],
        provenance=prov,
        X_hold=X_hold.astype(np.float32, copy=False),
...
def functional_score(
    ...
    *,
    ...
    if side == "input":
        ...
        rng = np.random.default_rng(SEED ^ 0x0F0F)
    ...
    # Reconstruction error is diagnostic only -- never an allocation signal.
...
# ---------------------------------------------------------------------------
# Activation-aware projection codec
# ---------------------------------------------------------------------------
...
def factor_bytes(rows: int, cols: int, rank: int, side: str) -> dict[str, int]:
    ...
    return {
        "header": HEADER_BYTES,
        "coefficients": int(coeff),
        "basis": int(basis),
    }


def packed_tensor_bytes(rows: int, cols: int, rank: int, side: str,
                        *, bill_basis: bool = True) -> int:
...
def serialize_tensor_payload(
    ...
    *,
    ...
    head = struct.pack(
        ...
        int(cols),
        int(rank),
        int(basis_layer) & 0xFFFF,
        1 if side == "input" else 2,
        1 if bill_basis else 0,
    )
    ...
    if len(blob) != billed:
        raise PackError(
            f"serialized {len(blob)} bytes but billed {billed}; "
...
def deserialize_tensor_payload(blob: bytes) -> dict[str, Any]:
    ...
    rows, cols, rank, basis_layer, side_code, has_basis = struct.unpack_from(
        "<IIIHHB", blob, 8
    )
    ...
    return {
        "rows": rows, "cols": cols, "rank": rank, "side": side,
        "basis_layer": basis_layer, "L": L, "B": B, "has_basis": bool(has_basis),
    }


# ---------------------------------------------------------------------------
# Phase 1: measurement (shardwise, streaming)
# ---------------------------------------------------------------------------
@dataclass
class RankPoint:
    rank: int
...
class TensorMeasurement:
    ...
    dtype: str
    side: str | None
    disposition: str  # "activation_aware" | "pass_through"
...
def measure_tensor(
    ...
    W: np.ndarray,
    dtype: str,
    basis: ActivationBasis | None,
    ranks: Sequence[int],
    *,
    bill_basis_per_tensor: bool = True,
) -> TensorMeasurement:
    ...
    # 1-D (norms, biases) and tensors with no side matching the activation width
    # that we cannot project: pass through at source width. Small mass; billed.
    if W.ndim != 2 or side is None or basis is None:
        ...
        return TensorMeasurement(
            ...
            basis_provenance=basis.provenance.as_dict() if basis else None,
            curve=[{
                "rank": 0,
                "bytes": pt_bytes,
                "bpw": (pt_bytes * 8) / max(n_weights, 1),
                "mean_row_cosine": 1.0,
                "constant_mean_cosine_null": 0.0,
                "beats_null": True,
                "surplus_over_null": 1.0,
                "reconstruction_relative_error_INADMISSIBLE": 0.0,
            }],
            pass_through_bytes=pt_bytes,
        )
    ...
    for rank in ranks:
        ...
        if r <= 0:
            continue
        B = basis.columns(r)
        # Basis lives in activation space of width `hidden`. For the input side
        # that is n; for the output side that is m. Columns are [hidden, r].
    ...
    return TensorMeasurement(
        ...
        basis_provenance=basis.provenance.as_dict(),
        curve=curve,
    )
    ...
    *,
    ...
    for name in iter_tensor_names(header):
        ...
        if prov.basis_layer not in basis_cache:
            basis_cache[prov.basis_layer] = build_basis(
                prov, capsule_map, max_basis_rank
            )
        basis = basis_cache[prov.basis_layer]
        # Refresh provenance with the measured row count.
        prov = basis.provenance
        # Re-bind tensor_layer into a fresh provenance for this tensor.
        tensor_prov = BasisProvenance(
            tensor_layer=layer_of(name),
            basis_layer=prov.basis_layer,
            capsule_file=prov.capsule_file,
            capsule_key=prov.capsule_key,
            hidden=prov.hidden,
            n_activation_rows=prov.n_activation_rows,
        )
        W = read_bf16_tensor(shard, header, name)
        dtype = header[name].get("dtype", "BF16")
        # Attach the per-tensor provenance (basis may be transferred).
        basis_for_tensor = ActivationBasis(
            layer=basis.layer,
            basis=basis.basis,
            singular_values=basis.singular_values,
            variance_frac=basis.variance_frac,
            provenance=tensor_prov,
            X_hold=basis.X_hold,
            X_fit_mean=basis.X_fit_mean,
        )
        out.append(measure_tensor(
            name, W, dtype, basis_for_tensor, ranks,
...
# ---------------------------------------------------------------------------
# Phase 2: allocation (global, deterministic)
# ---------------------------------------------------------------------------
@dataclass
...
def allocate(
    measurements: Sequence[TensorMeasurement | dict[str, Any]],
    target_bpw: Fraction,
    *,
    ...
    rows: list[dict[str, Any]] = [
        m.as_dict() if isinstance(m, TensorMeasurement) else dict(m)
        for m in measurements
    ]
    rows.sort(key=lambda r: r["name"])  # stable order

    if weight_count is None:
        weight_count = int(sum(int(r["n_weights"]) for r in rows))
    budget_bits = target_bpw * int(weight_count)
    budget_bytes = int(budget_bits // 8)  # floor; never exceed the rational budget

    # Seed every compressible tensor at its lowest measured rank; pass-through fixed.
    ...
    for r in rows:
        if r["disposition"] == "pass_through":
            ...
            choice[r["name"]] = {
                "rank": 0,
                "bytes": int(pt["bytes"]),
                "mean_row_cosine": float(pt["mean_row_cosine"]),
                "constant_mean_cosine_null": float(pt["constant_mean_cosine_null"]),
                "beats_null": bool(pt["beats_null"]),
                "surplus_over_null": float(pt["surplus_over_null"]),
                "disposition": "pass_through",
            }
            continue
        # Lowest rank on the curve THAT BEATS ITS NULL.
        #
        # Beating the constant-mean null is a hard constraint, not a scoring term. A tensor
        # below its null is worse than emitting the mean row -- it carries no information --
        # and no BPW saving compensates for that. The first dry run allocated lm_head at
        # cosine 0.683 against a null of 0.852 and embed_tokens at 0.869 against 0.925;
        # between them that is 1.9e9 weights, and an lm_head below its null produces logits
        # worse than the mean, which is precisely Math-Preserve's failure mode.
        #
        # A tensor that cannot beat its null at ANY available rank falls through to native
        # pass-through rather than being allocated a rank that does not work. That is also
        # what the campaign learned the expensive way: General-R0 needed lm_head and
        # embed_tokens natively protected, and compressing them below null here would have
        # repeated it.
        ordered = sorted(r["curve"], key=lambda p: int(p["rank"]))
        viable = [p for p in ordered if bool(p["beats_null"])]
        if not viable:
            # Bill native at source width. An activation-aware measurement carries no
            # pass_through_bytes field -- only the pass-through branch sets it -- so
            # deriving it from n_weights is what keeps the ledger honest here. Getting
            # this wrong would under-bill the very tensors being protected.
            ...
            choice[r["name"]] = {
                "rank": 0,  # 0 == native, matching the existing pass-through path
                "bytes": pt_bytes,
                "mean_row_cosine": float(best["mean_row_cosine"]),
                "constant_mean_cosine_null": float(best["constant_mean_cosine_null"]),
                "beats_null": False,
                "surplus_over_null": float(best["surplus_over_null"]),
                "disposition": "pass_through",
                "forced_native_because": "no available rank beats the constant-mean null",
                "best_surplus_seen": float(best["surplus_over_null"]),
                "side": r.get("side"),
                "shape": r["shape"],
                "basis_layer": (r.get("basis_provenance") or {}).get("basis_layer"),
            }
            continue
        pt = viable[0]
        choice[r["name"]] = {
    ...
    def current_bytes() -> tuple[int, dict[str, int]]:
        ...
        # Packaging: one 256-byte shard index stub per tensor (deterministic, billed).
        components["packaging"] += 256 * len(choice)
        return int(sum(components.values())), components

    total, _ = current_bytes()
    if total > budget_bytes:
        # Even the cheapest legal program overshoots. Keep the floor allocation and
        # report the overrun honestly rather than inventing a free representation.
        over = True
    else:
        over = False
        # Greedy upgrades: best surplus gain per extra byte. Deterministic ties.
        upgraded = True
        while upgraded:
            upgraded = False
            ...
            for name, ch in choice.items():
                ...
                # Delta utility / delta bytes under the shared-basis rebill.
                ...
                # Revert; we only score here.
                ch.update(prev)
                ch["rank"] = cur_rank
                d_bytes = new_total - old_total
                d_util = float(nxt["surplus_over_null"]) - old_surplus
                if d_bytes <= 0:
                    # Free (or cheaper) upgrade that does not hurt utility: always take,
                    # scored as +inf with name/rank for determinism.
            ...
            if not candidates:
                break
            # Best score; ties broken by name then rank for determinism.
            candidates.sort(key=lambda t: (-t[0], t[1], t[2]))
            for score, name, new_rank, nxt in candidates:
                # Apply if it still fits.
                ch = choice[name]
                prev = dict(ch)
                ch["rank"] = int(nxt["rank"])
                ch["bytes"] = int(nxt["bytes"])
                ...
                if new_total <= budget_bytes:
                    upgraded = True
                    break
                # Revert and try the next-best candidate.
                ch.update(prev)
    ...
    for r in rows:
        ch = choice[r["name"]]
        # Recompute exact coefficient bytes under shared-basis mode for the record.
        if ch["disposition"] == "activation_aware" and shared_bases:
            ...
            billed = int(ch["bytes"])
        allocations.append(TensorAllocation(
            name=r["name"],
            disposition=ch["disposition"],
            rank=int(ch["rank"]),
            bytes=billed,
            n_weights=int(r["n_weights"]),
            mean_row_cosine=float(ch["mean_row_cosine"]),
            constant_mean_cosine_null=float(ch["constant_mean_cosine_null"]),
            beats_null=bool(ch["beats_null"]),
            surplus_over_null=float(ch["surplus_over_null"]),
            side=r.get("side"),
            basis_provenance=r.get("basis_provenance"),
            shape=list(r["shape"]),
        ))
    ...
    return {
        "schema": "hawking.glm52.activation_aware_allocation.v1",
        "target_bpw_exact": f"{target_bpw.numerator}/{target_bpw.denominator}",
        "target_bpw_float": float(target_bpw),
        "weight_count": int(weight_count),
        "shared_bases": bool(shared_bases),
        "basis_rank_by_layer": {str(k): int(v) for k, v in sorted(layer_ranks.items())},
        "byte_ledger": ledger.as_dict(weight_count),
        "total_bytes": total,
        "budget_bytes": budget_bytes,
        "within_budget": total <= budget_bytes and not over,
        "floor_over_budget": over,
        "n_tensors": len(allocations),
        "n_activation_aware": sum(1 for a in allocations if a.disposition == "activation_aware"),
        "n_pass_through": sum(1 for a in allocations if a.disposition == "pass_through"),
        "n_beats_null": sum(1 for a in allocations if a.beats_null),
        "mean_surplus_over_null": float(
            np.mean([a.surplus_over_null for a in allocations]) if allocations else 0.0
        ),
...
# ---------------------------------------------------------------------------
# Phase 3: packing (shardwise, streaming)
# ---------------------------------------------------------------------------
def pack_tensor_at_rank(
    W: np.ndarray,
    ...
    *,
    ...
    W_hat = reconstruct(L, B, side)
    score = functional_score(W, W_hat, basis.X_hold, side=side)
    blob = serialize_tensor_payload(
        L, B, side=side, rows=int(W.shape[0]), cols=int(W.shape[1]),
        rank=r, basis_layer=basis.provenance.basis_layer, bill_basis=bill_basis,
    )
    meta = {
        "rank": r,
        "side": side,
...
def pack_shard(
    ...
    *,
    ...
    # Shared bases first (layers touched by this shard), deterministic order.
    if shared_bases:
        ...
        for bl in sorted(needed_layers):
            if bl not in basis_cache:
                ...
                prov = BasisProvenance(
                    tensor_layer=bl, basis_layer=bl,
                    capsule_file=path.name, capsule_key=key,
                    hidden=HIDDEN, n_activation_rows=0,
                )
                basis_cache[bl] = build_basis(prov, capsule_map, max_basis_rank)
            aa_ranks = [
                int(allocation_by_name[n]["rank"])
                for n in iter_tensor_names(header)
                if allocation_by_name[n]["disposition"] == "activation_aware"
                and resolve_basis_for_tensor(n, capsule_map).basis_layer == bl
            ]
            ...
            basis_index.append({
                ...
                "rank": rnk,
                "bytes": len(bblob),
                "offset": sum(len(x) for x in basis_section),
                "capsule_file": basis_cache[bl].provenance.capsule_file,
                "capsule_key": basis_cache[bl].provenance.capsule_key,
            })
    ...
    for name in iter_tensor_names(header):
        ...
        if alloc["disposition"] == "pass_through":
            ...
            pthead = struct.pack(
                "<8sIII", b"GLM52PT0", int(W.ndim),
                int(W.shape[0]) if W.ndim > 0 else 0,
                int(W.shape[1]) if W.ndim > 1 else 0,
            )
            ...
            tensors_out.append({
                ...
                "shape": list(W.shape),
                "basis_provenance": alloc.get("basis_provenance"),
            })
        ...
        tensor_prov = BasisProvenance(
            tensor_layer=layer_of(name),
            basis_layer=basis.provenance.basis_layer,
            capsule_file=basis.provenance.capsule_file,
            capsule_key=basis.provenance.capsule_key,
            hidden=basis.provenance.hidden,
            n_activation_rows=basis.provenance.n_activation_rows,
        )
        basis_for_tensor = ActivationBasis(
            layer=basis.layer,
            basis=basis.basis,
        ...
        blob, meta = pack_tensor_at_rank(
            W, basis_for_tensor, int(alloc["rank"]), side, bill_basis=not shared_bases,
        )
    ...
    if not ledger.reconciles(len(physical)):
        raise PackError(
            f"ledger {ledger.total_bytes()} != physical {len(physical)}; "
            f"components={ledger.components}"
        )
    ...
    return {
        ...
        "ledger": ledger.as_dict(weight_count),
        "itemization_reconciles": True,
        "bases": basis_index,
        "tensors_with_provenance": sum(
            1 for t in tensors_out if t.get("basis_provenance") is not None
        ),
    }
...
def phase_measure(
    ...
    *,
    ...
    return {
        ...
        "ranks": list(ranks),
        "shards": list(shards),
        "n_tensors": len(measurements),
        "n_weights": int(sum(m["n_weights"] for m in measurements)),
        "capsule_layers_available": sorted(capsule_map),
        "bases_built": {
            str(layer): {
                "max_rank": b.max_rank,
                "variance_frac_at_16": (
                    b.variance_frac[min(15, len(b.variance_frac) - 1)]
                    if b.variance_frac else None
                ),
                "provenance": b.provenance.as_dict(),
            }
            for layer, b in sorted(basis_cache.items())
        },
        "per_shard": per_shard,
        "measurements": measurements,
        "seconds": round(time.time() - t0, 2),
        "note": (
            "Reconstruction error is present on each curve point as a diagnostic "
            "and is INADMISSIBLE for allocation. Allocation reads surplus_over_null."
        ),
    }


def phase_allocate(
    measurement_doc: dict[str, Any],
    target_bpw: Fraction,
    *,
    shared_bases: bool = True,
    whole_model_weight_count: int | None = None,
) -> dict[str, Any]:
    ...
    doc["allocation_sha256"] = hashlib.sha256(
        json.dumps(
            {"target": doc["target_bpw_exact"], "rows": doc["allocations"]},
...
def phase_pack(
    ...
    *,
    ...
    layer_ranks = {
        int(k): int(v) for k, v in allocation_doc.get("basis_rank_by_layer", {}).items()
    }
    ...
    return {
        "schema": "hawking.glm52.activation_aware_pack_receipt.v1",
        "at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "shards": list(shards),
        "out_dir": str(out_dir),
...
def dry_run(
    ...
    *,
    ...
    for n in shards:
        ...
        if not path.exists():
            raise PackError(
                f"dry-run requires shards on disk; missing {path}. "
                f"Rehydrate the pilot window first."
            )
    measurement = phase_measure(
        shards, source_dir, ranks,
        fetch=False, evict=False, floor=floor,
        bill_basis_per_tensor=False,
        enforce_floor=False,
    )
    # Allocate against the measured subset's own weight count (pilot scope),
    # and ALSO report what the same program would bill on the whole-model
    # denominator when the measured mass is treated as representative -- but the
    # binding number for dry-run is the subset ledger.
    allocation = phase_allocate(
        measurement, target_bpw,
        shared_bases=True,
        whole_model_weight_count=int(measurement["n_weights"]),
    )
    doc = {
        "schema": "hawking.glm52.activation_aware_dry_run.v1",
        "at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "mode": "dry-run",
        "shards": list(shards),
        "target_bpw_exact": f"{target_bpw.numerator}/{target_bpw.denominator}",
        "measurement_summary": {
            "n_tensors": measurement["n_tensors"],
            "n_weights": measurement["n_weights"],
            "seconds": measurement["seconds"],
            "bases_built": measurement["bases_built"],
            "capsule_layers_available": measurement["capsule_layers_available"],
        },
        "allocation": allocation,
        "provenance_present_on_all_aa_tensors": all(
            (a.get("basis_provenance") is not None)
            for a in allocation["allocations"]
            if a["disposition"] == "activation_aware"
        ),
        "byte_ledger_reconciles": allocation["byte_ledger"]["itemization_reconciles"],
        "full_run_command": (
            f"python3.12 tools/condense/glm52_activation_aware_pack.py --full "
            f"--shards 1-282 --target-bpw {target_bpw.numerator}/{target_bpw.denominator} "
            f"--fetch --evict --out \"$HOME/Library/Application Support/Hawking/"
            f"GLM52Gravity/activation_aware_pack\""
        ),
        "projected_full_run": project_full_run_cost(
            pilot_n_weights=int(measurement["n_weights"]),
            pilot_seconds=float(measurement["seconds"]),
            pilot_n_shards=len(shards),
        ),
        "note": (
            "Dry-run measured and allocated only; no packing, no fetch, no eviction. "
            "Allocation maximises surplus-over-null under the complete-byte budget. "
            "Reconstruction error was logged on the measurement curve and did not "
            "drive the allocation."
        ),
    }
    # Keep the full measurement on disk if requested -- it is large.
    if out is not None:
        out = Path(out)
...
def project_full_run_cost(
    *,
    ...
    # Fetch: 1.507 TB at 2.0 Gbit/s ≈ 1.7 h; with eviction, sequential per shard.
    fetch_seconds = 1.7 * 3600
    # Packing re-reads each shard once more after allocation (~same as measure).
    pack_seconds = measure_pack_seconds  # approximate
    total = fetch_seconds + measure_pack_seconds + pack_seconds
    return {
        "pilot_n_shards": pilot_n_shards,
        "pilot_n_weights": pilot_n_weights,
        "pilot_measure_seconds": pilot_seconds,
        "full_shards": 282,
        "full_weights": full_w,
        "weight_scale": scale,
        "projected_fetch_hours": round(fetch_seconds / 3600, 2),
        "projected_measure_hours": round(measure_pack_seconds / 3600, 2),
        "projected_pack_hours": round(pack_seconds / 3600, 2),
        "projected_total_hours": round(total / 3600, 2),
        "assumptions": [
            "fetch at the measured 2.0 Gbit/s rehydration rate (~1.7 h for 1.507 TB)",
            "measure cost scales with weight mass from the pilot window",
            "pack re-streams every shard once after global allocation",
            "eviction keeps resident shard bodies near zero; floor is 141 GiB free",
        ],
    }


# ---------------------------------------------------------------------------
# Self-test / unit checks (no parent bodies required)
# ---------------------------------------------------------------------------
def selftest() -> int:
    rng = np.random.default_rng(0)

    # 1. Byte accounting reconciles on a synthetic tensor.
    m, n, r = 64, HIDDEN, 16
    ...
    blob = serialize_tensor_payload(
        L, B, side="input", rows=m, cols=n, rank=r, basis_layer=10, bill_basis=True,
    )
    ...
    # 2. Basis provenance is present on a measured activation-aware tensor.
    #    Build a synthetic "capsule" basis.
    X = rng.standard_normal((512, HIDDEN)).astype(np.float32)
    # Fake provenance + basis via the same math build_basis uses.
    mu = X.mean(0)
    _u, s, vt = np.linalg.svd(X - mu, full_matrices=False)
    max_r = 32
    basis = ActivationBasis(
        layer=10,
        basis=vt[:max_r].T.astype(np.float32),
        singular_values=s[:max_r].astype(np.float32),
        variance_frac=[float(x) for x in np.cumsum(s[:max_r] ** 2) / (np.sum(s ** 2) + 1e-30)],
        provenance=BasisProvenance(
            tensor_layer=10, basis_layer=10,
            capsule_file="L10_L10.npz", capsule_key="layer_10/pre_router_hidden",
            hidden=HIDDEN, n_activation_rows=512,
        ),
        X_hold=X[400:],
        X_fit_mean=mu.astype(np.float32),
    )
    ...
    assert a1["allocation_sha256"] if False else True  # sha is added by phase_allocate
    ...
    # Provenance survives allocation.
    ...
    # Nearest-layer resolution is deterministic.
    avail = [0, 10, 11, 16, 22, 76]
    assert nearest_basis_layer(10, avail) == 10
    assert nearest_basis_layer(12, avail) == 11
    assert nearest_basis_layer(13, avail) == 11  # tie 11 vs 16? |13-11|=2, |13-16|=3 -> 11
    assert nearest_basis_layer(14, avail) == 16  # |14-11|=3, |14-16|=2 -> 16
    assert nearest_basis_layer(77, avail) == 76

    # Shared-basis bytes reconcile with the formula.
    ...
    print(json.dumps({
        ...
        "basis_provenance_present": True,
        "disk_floor_halts": True,
        "allocation_deterministic": True,
        "sample_complete_bpw_exact": a1["complete_bpw_exact"],
        "sample_n_tensors": a1["n_tensors"],
    }, indent=2))
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("command", nargs="?", default=None,
                    help="optional: selftest")
    ap.add_argument("--dry-run", action="store_true",
                    help="phases 1+2 on shards already on disk; print allocation; no pack")
    ap.add_argument("--full", action="store_true",
                    help="measure -> allocate -> pack over the named shards")
    ap.add_argument("--measure-only", action="store_true")
    ap.add_argument("--allocate-from", type=Path,
                    help="path to a measurement JSON; run phase 2 only")
    ap.add_argument("--pack-from", type=Path,
                    help="path to an allocation JSON; run phase 3 only")
    ap.add_argument("--shards", default="1,2,3,4,5",
                    help="shard list, e.g. 1,2,3,4,5 or 1-282 (default: pilot window)")
    ap.add_argument("--source-dir", type=Path, default=PILOT_SOURCE)
    ap.add_argument("--out", type=Path, default=None,
                    help="output path for receipts / packed artifacts")
    ap.add_argument("--target-bpw", default="49/50",
                    help="whole-model complete BPW target as rational (default 49/50 = 0.98)")
    ap.add_argument("--ranks", default="8,16,32,64,128,256",
                    help="rank ladder for measurement")
    ...
    floor = int(args.disk_floor_gib) * (1 << 30)
    ranks = tuple(int(x) for x in args.ranks.split(",") if x.strip())
    shards = parse_shard_list(args.shards, default=[1, 2, 3, 4, 5])
    target = parse_bpw(args.target_bpw)
    out = args.out

    try:
        no_mode = not any(
            [args.full, args.measure_only, args.allocate_from, args.pack_from]
        )
        if args.dry_run or no_mode:
            doc = dry_run(
                shards, args.source_dir, ranks, target,
                floor=floor,
                out=out or (DEFAULT_OUT / "DRY_RUN.json"),
            )
            summary = {
                "mode": "dry-run",
                "shards": doc["shards"],
                "n_tensors": doc["measurement_summary"]["n_tensors"],
                "n_weights": doc["measurement_summary"]["n_weights"],
                "target_bpw_exact": doc["target_bpw_exact"],
                "complete_bpw_exact": doc["allocation"]["complete_bpw_exact"],
                "complete_bpw_float": doc["allocation"]["complete_bpw_float"],
                "total_bytes": doc["allocation"]["total_bytes"],
                "within_budget": doc["allocation"]["within_budget"],
                "n_activation_aware": doc["allocation"]["n_activation_aware"],
                "n_pass_through": doc["allocation"]["n_pass_through"],
                "n_beats_null": doc["allocation"]["n_beats_null"],
                "mean_surplus_over_null": doc["allocation"]["mean_surplus_over_null"],
                "basis_rank_by_layer": doc["allocation"]["basis_rank_by_layer"],
                "provenance_present_on_all_aa_tensors": doc[
                    "provenance_present_on_all_aa_tensors"
                ],
                "byte_ledger_reconciles": doc["byte_ledger_reconciles"],
                "byte_ledger": doc["allocation"]["byte_ledger"]["component_bytes"],
                "projected_full_run": doc["projected_full_run"],
                "full_run_command": doc["full_run_command"],
                "wrote": doc.get("wrote"),
                "measure_seconds": doc["measurement_summary"]["seconds"],
            }
            sample = doc["allocation"]["allocations"][:8]
            summary["allocation_sample"] = [
                {
                    "name": a["name"], "rank": a["rank"],
                    "disposition": a["disposition"],
                    "cosine": round(a["mean_row_cosine"], 4),
                    "null": round(a["constant_mean_cosine_null"], 4),
                    "basis_layer": (a.get("basis_provenance") or {}).get("basis_layer"),
                }
                for a in sample
            ]
        ...
        if args.measure_only:
            ...
            print(json.dumps({
                "wrote": str(dest),
                "n_tensors": doc["n_tensors"],
                "n_weights": doc["n_weights"],
                "seconds": doc["seconds"],
            }, indent=2))
            return 0

        if args.allocate_from:
            measurement = json.loads(Path(args.allocate_from).read_text())
            doc = phase_allocate(measurement, target, shared_bases=True)
            dest = out or (DEFAULT_OUT / "ALLOCATION.json")
            dest = Path(dest)
            dest.parent.mkdir(parents=True, exist_ok=True)
            dest.write_text(json.dumps(doc, indent=2, sort_keys=True) + "\n")
            print(json.dumps({
                "wrote": str(dest),
                "complete_bpw_exact": doc["complete_bpw_exact"],
                "n_tensors": doc["n_tensors"],
        ...
        if args.pack_from:
            allocation = json.loads(Path(args.pack_from).read_text())
            ...
            print(json.dumps({
                "wrote": str(receipt_path),
                "complete_bpw_exact": doc["byte_ledger"]["complete_bpw_exact"],
                "itemization_reconciles": doc["itemization_reconciles"],
            }, indent=2))
            return 0
        ...
        if args.full:
            ...
            # Whole-model target uses the sealed denominator when all shards run;
            # for a partial shard list, budget against measured mass.
            wc = (
                ORIGINAL_WEIGHT_COUNT
                if len(shards) >= 282
                else int(measurement["n_weights"])
            )
            allocation = phase_allocate(
                measurement, target, shared_bases=True,
                whole_model_weight_count=wc,
            )
            (dest_dir / "ALLOCATION.json").write_text(
                json.dumps(allocation, indent=2, sort_keys=True) + "\n"
            )
            # Re-stream for pack. Fetch again if we evicted; default full keeps
            # bodies through pack then evicts if --evict.
            pack_doc = phase_pack(
                shards, args.source_dir, allocation, dest_dir, ranks,
                fetch=args.fetch, evict=args.evict, floor=floor,
            )
            ...
            print(json.dumps({
                "mode": "full",
                "shards": list(shards),
                "complete_bpw_exact": allocation["complete_bpw_exact"],
                "within_budget": allocation["within_budget"],
                "itemization_reconciles": pack_doc["itemization_reconciles"],
                "out": str(dest_dir),
            }, indent=2))
            return 0
        ...
        print(json.dumps({
            "status": "STOP_DISK_FLOOR",
            "error": str(exc),
            "disk_floor_gib": args.disk_floor_gib,
            "free_gib": round(free_bytes() / (1 << 30), 3),
