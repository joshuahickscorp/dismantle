...
def _catalog() -> tuple[dict[str, Any], ...]:
    ...
    return (
        ...
        _item(
            ...
            detail="the archived n-gram school result is represented by canonical receipts; the next unit is a fresh candidate set scored against the negative index, not a GPU fit.",
            required_lanes=(LANE_CPU, LANE_REPRESENTATION),
            gain=INFO_MEDIUM,
            species="learned_compiler_experiment",
            verifier="receipts/future/NGRAM_SCHOOL.json",
            evidence=(f"{ev}/NGRAM_SCHOOL.json", f"{ev}/NEGATIVE_SCIENCE_INDEX.json"),
            hypothesis_family="representation_school_candidates",
        ),
        _item(
