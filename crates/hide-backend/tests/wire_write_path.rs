...
async fn save_through_the_wire_path_records_a_diff_and_publishes_it() {
    ...
    assert_eq!(std::fs::read_to_string(dir.join("pool.rs")).unwrap(), "fn pool() { /* capped */ }\n");
    let proposal = host
        .diff_get(&diff_id)
    ...
    assert_eq!(std::fs::read_to_string(dir.join("retry.rs")).unwrap(), "fn retry() {}\n");
    assert_eq!(std::fs::read_to_string(dir.join("pool.rs")).unwrap(), "fn pool() { /* capped */ }\n");
...
async fn a_code_rewind_reverts_a_wire_save_on_disk() {
    ...
    .await
    .unwrap();
    assert_eq!(std::fs::read_to_string(&file).unwrap(), "at the checkpoint\n");
    let _ = std::fs::remove_dir_all(dir);
}
#[tokio::test]
...
fn the_new_wire_names_are_on_the_contract() {
 ...
 assert_eq!( BackendHost::editor_run(&session), RunId::from("editor-ses_test") );
}
