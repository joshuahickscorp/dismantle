...
└── workspace/                    # non-runtime project material (five role-based areas)
    ├── campaign/
    │   ├── config/               # adapters, packs, preregistrations, profiles, prompts
    │   ├── evidence/             # model, runtime, systems, and research evidence
    │   ├── governance/
    │   │   ├── control/          # catalog, ledgers, verdicts, receipts, rungs
    │   │   ├── odyssey/          # domains, program, resources, state, records
    │   │   └── receipts/         # immutable governance receipts
    │   └── records/              # reports and local reproducible research output
    ├── docs/                     # reference, guides, implementation notes, plans, history
    ├── ops/                      # deployment, local scripts, build output, logs
    ├── quality/                  # repository-level fixtures
    └── vendor/                   # third-party source retained in-tree
```

**Campaign material does not live at the repository root.** Artifacts go in
`workspace/campaign/evidence/<area>/<campaign>/`, named for the
`hawking.<family>.*` schema they carry.
Anything a tool can rebuild — the semantic graph, its analyses, the viewer —
is build output and belongs in `workspace/ops/build/`, not in evidence and not
at the root. The root holds only build manifests and executable entry points.
...
strings, and the readers resolve the campaign evidence tree themselves
