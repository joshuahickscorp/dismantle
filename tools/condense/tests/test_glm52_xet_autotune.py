...
def _prepared_autotune_intent(plan: dict, expected_contract_sha256: str = _AUTH_CONTRACT) -> dict:
    claim_id = "test:xet-autotune:authority:0001"
    anchor_body = {
        "schema": state.CONTROLLER_ANCHOR_SCHEMA, "campaign_id": _AUTH_CAMPAIGN,
        "source_revision": autotune.REVISION, "controller_epoch": autotune.CONTROLLER_EPOCH,
        "expected_contract_sha256": expected_contract_sha256, "from_state": "BUILD_DEPENDENCY_GRAPH",
        "checkpoint": {
            "event_count": 6, "event_head_hash": "1" * 64, "window_event_count": 0,
            "window_event_head_hash": state.GENESIS_HASH, "checkpoint_seal_sha256": "2" * 64,
        },
    }
    anchor = {**anchor_body, "anchor_sha256": autotune._canonical_sha256(anchor_body)}
    terminal = seal({
        "schema": state.TERMINAL_EVIDENCE_SCHEMA, "state": "AUTOTUNE_XET",
        "expected_contract_sha256": expected_contract_sha256,
        "controller_anchor_sha256": anchor["anchor_sha256"],
        "artifact_seals": {
            "xet_autotune_plan": {
                "path": "GLM52_XET_AUTOTUNE_PLAN.json", "file_sha256": "3" * 64,
                "seal_sha256": plan["seal_sha256"], "schema": autotune.PLAN_SCHEMA,
                "status": "PASS_OFFLINE_PLAN_BODY_NOT_READ",
            },
        },
        "checklist": {}, "phone_status": None, "created_at": "2026-07-21T12:00:00Z",
    })
    ...
    request_sha256 = autotune._canonical_sha256({
        ...
        "campaign_id": _AUTH_CAMPAIGN, "source_revision": autotune.REVISION,
        "controller_epoch": autotune.CONTROLLER_EPOCH,
        "expected_contract_sha256": expected_contract_sha256, "claim_id": claim_id,
        "to_state": "AUTOTUNE_XET", "state_payload": state_payload,
    })
    status = {
        "state": "AUTOTUNE_XET", "source_coverage_percent": 0.0,
        "shards": {"fetched": 0, "verified": 0, "evicted": 0, "total": 282},
        "network_bytes": 0, "throughput_bytes_per_second": 0.0, "eta_seconds": None,
        "current": {"window": None, "layer": None}, "candidate_rates": [], "best_metrics": {},
        "resources": {
            "disk_free_bytes": 640_000_000_000, "ram_available_bytes": 90_000_000_000,
            "swap_used_bytes": 0,
        },
    ...
    dedupe_key = autotune._canonical_sha256({
        ...
        "campaign_id": _AUTH_CAMPAIGN, "source_revision": autotune.REVISION,
        "controller_epoch": autotune.CONTROLLER_EPOCH,
        "expected_contract_sha256": expected_contract_sha256, "event_kind": event_kind,
        "claim_id": claim_id, "from_state": "BUILD_DEPENDENCY_GRAPH", "to_state": "AUTOTUNE_XET",
    ...
    rendered = state.render_campaign_status_message(
        event_kind, dedupe_key, status, anchor, claim_id=claim_id,
        from_state="BUILD_DEPENDENCY_GRAPH", to_state="AUTOTUNE_XET",
    )
    sealed_intent = seal({
        "schema": state.TRANSITION_INTENT_SCHEMA, "campaign_id": _AUTH_CAMPAIGN,
        "source_revision": autotune.REVISION, "controller_epoch": autotune.CONTROLLER_EPOCH,
        "expected_contract_sha256": expected_contract_sha256, "event_kind": event_kind,
        "from_state": "BUILD_DEPENDENCY_GRAPH", "to_state": "AUTOTUNE_XET",
        "claim_id": claim_id, "requested_payload": requested_payload,
        "state_payload": state_payload, "request_sha256": request_sha256,
        "dedupe_key": dedupe_key, "controller_anchor": anchor,
        "canonical_status": status, "canonical_status_sha256": status_sha256,
        "rendered_message": rendered,
        "rendered_message_sha256": autotune.hashlib.sha256(rendered.encode("utf-8")).hexdigest(),
...
def _authority(plan: dict, expected_contract_sha256: str = _AUTH_CONTRACT) -> dict:
    ...
    receipt = state.make_telegram_delivery_receipt(
        intent, auth=_auth_config(),
        bot_api_response={"ok": True, "result": {
            "message_id": 9, "chat": {"id": _AUTH_CHAT_ID, "type": "private"},
            "text": intent["rendered_message"],
        }},
        http_status=200, delivered_at="2026-07-21T12:00:01Z",
    )
    ac = intent["controller_anchor"]["checkpoint"]
    checkpoint = {
        "schema": autotune.COMMITTED_CHECKPOINT_REF_SCHEMA,
        "checkpoint_schema": state.CHECKPOINT_SCHEMA, "campaign_id": _AUTH_CAMPAIGN,
        "source_revision": autotune.REVISION, "controller_epoch": autotune.CONTROLLER_EPOCH,
        "expected_contract_sha256": expected_contract_sha256, "state": "AUTOTUNE_XET",
        "last_claim_id": intent["claim_id"], "transition_intent_sha256": intent["seal_sha256"],
        "telegram_receipt_hmac_sha256": receipt["hmac_sha256"], "checkpoint_seal_sha256": "8" * 64,
        "event_count": ac["event_count"] + 1, "event_head_hash": "9" * 64,
        "window_event_count": ac["window_event_count"],
        "window_event_head_hash": ac["window_event_head_hash"],
    }
    return seal({
        "schema": autotune.EXECUTION_AUTHORITY_SCHEMA,
        "status": "AUTHORIZED_BY_COMMITTED_AUTOTUNE_XET_TRANSITION",
        "repo": autotune.REPO_ID, "revision": autotune.REVISION, "campaign_id": _AUTH_CAMPAIGN,
        "controller_epoch": autotune.CONTROLLER_EPOCH,
        "expected_contract_sha256": expected_contract_sha256, "plan_seal_sha256": plan["seal_sha256"],
        "transition_intent": intent, "telegram_delivery_receipt": receipt,
        "committed_controller_checkpoint": checkpoint, "credentials_serialized": False,
    })

class CurrentStateAuthorityVerifier:
    """Test double with independent auth, checkpoint, and lease observations."""

    def __init__(self, authority: dict, *, fail: str | None = None, lease_held: bool = True) -> None:
        self.fail = fail
    ...
    def verify_prepared_transition_intent_hmac(
        self, transition_intent: dict, *, plan_seal: str, expected_contract_sha256: str,
    ) -> bool:
        try:
            state.validate_transition_intent(transition_intent, _auth_config())
        except state.StateError:
            return False
        return self._trusted_args(plan_seal, expected_contract_sha256) and self._result("intent")

    def verify_telegram_delivery_receipt(
        self, receipt: dict, *, transition_intent: dict, plan_seal: str,
        expected_contract_sha256: str,
    ) -> bool:
        try:
            state.validate_telegram_delivery_receipt(receipt, transition_intent, _auth_config())
        except state.StateError:
            return False
        return self._trusted_args(plan_seal, expected_contract_sha256) and self._result("telegram")

    def verify_committed_controller_checkpoint(
        self, checkpoint: dict, *, transition_intent: dict, telegram_receipt: dict,
        plan_seal: str, expected_contract_sha256: str,
    ) -> bool:
        bound = (
        ...
        return bound and self._trusted_args(plan_seal, expected_contract_sha256) and self._result("checkpoint")

    def verify_live_singleton_lease(
        self, checkpoint: dict, *, transition_intent: dict, plan_seal: str,
        expected_contract_sha256: str,
    ) -> bool:
        bound = (
            checkpoint == self.live_checkpoint
