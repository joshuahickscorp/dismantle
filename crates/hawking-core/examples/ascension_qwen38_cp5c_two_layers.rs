//! CP5c — TWO layers chained. Does a chunk cross a layer boundary?
//!
//! CP5b showed one layer batches at 2.205x with the real recurrence serial
//! inside the command buffer. What it could not show is a chunk crossing from
//! layer L to layer L+1, which is the last structural question before
//! integration and the one `CP5B_RESULT.md` names as outstanding.
//!
//! There is a specific claim to test, and it decides the integration cost model:
//!
//!   the multi-position OUTPUT layout is `out[row * K + k]`
//!   the multi-position INPUT  layout is `input[col * K + k]`
//!
//! down_proj emits 5120 rows, which is the hidden size, and the next layer's
//! in_proj contracts over 5120 columns. So layer L's output is ALREADY in layer
//! L+1's input layout, and the interleave is paid ONCE at the top of the chunk
//! rather than once per layer. If that is true the per-layer transform cost that
//! CP3 charged at 15.9% of the GPU time it saved vanishes for every layer after
//! the first. If it is false, integration pays a transpose 64 times.
//!
//!   SEQUENTIAL  for each position: layer1 then layer2, one command buffer per
//!               position.  K CBs, 8K dispatches.
//!   BATCHED     ONE command buffer: layer1 for all K, then layer2 for all K.
//!               1 CB, 6 + 2K dispatches.
//!
//! Layer 2 consumes layer 1's down output DIRECTLY -- no re-interleave, no copy.
//! That the outputs still match position by position is what proves the layouts
//! line up; if they did not, the second layer would read a transposed chunk and
//! the comparison would fail loudly rather than silently.
//!
//! Build:
//!   cargo build --profile release-fast -p hawking-core \
//!     --example ascension_qwen38_cp5c_two_layers
//! Timing (REQUIRES zero loaded 27B residents):
//!   ./tools/gpu_lane_lock.sh cp5c .../ascension_qwen38_cp5c_two_layers \
//!     --timing --reps 9 --out receipts/runtime/CP5C_TWO_LAYERS.json
...
mod macos {
    ...
    const SCHEMA: &str = "hawking.cp5c.two_layers.v1";
    ...
    struct Rec {
        state: metal::Buffer,
        /// Layer 2's own state. Sharing one buffer across layers was a HARNESS
        /// bug that produced max_rel 2.342e2: the sequential arm interleaves
        /// (p0L1, p0L2, p1L1, p1L2, ...) while the batched arm runs (L1 x K,
        /// then L2 x K), so a shared order-dependent state diverges by
        /// construction. Every DeltaNet layer in the real model carries its own
        /// recurrent state; the harness now does too.
        state2: metal::Buffer,
        state_seed: Vec<u8>,
        q: Vec<metal::Buffer>,
        k: Vec<metal::Buffer>,
        ...
        out_seq2: Vec<metal::Buffer>,
        out_bat2: Vec<metal::Buffer>,
    }
    ...
    struct Out {
        ...
        inp2: Vec<metal::Buffer>, gate2: Vec<metal::Buffer>,
        up2: Vec<metal::Buffer>, down2: Vec<metal::Buffer>,
    }
    ...
    fn seq_one_position(
        ...
        // LAYER 2 READS LAYER 1's OUTPUT inside this command buffer, so the
        // dispatches must be ORDERED against each other. Without a serial group
        // the second layer's projections differed from the sequential arm by
        // ~2e-3 on inputs that were bit-identical -- a read-before-write race,
        // not a numerical one.
        ...
        dispatch_rec_on(&mut tcb, r, &r.state, k, &r.out_seq[k])?;
        ...
        // ---- LAYER 2, in the SAME command buffer, reading layer 1's down
        // output directly. No copy and no re-interleave: down emits 5120 values
        // for this position and in_proj contracts over 5120 columns.
        tcb.dispatch_threads(SEQ_INPROJ, ((((IN_ROWS as u32) + 1) / 2) * 128, 1, 1), tg, |enc| {
            enc.set_buffer(0, Some(&w.in_codes), 0); enc.set_buffer(1, Some(&w.in_scales), 0);
            enc.set_buffer(2, Some(&o.down[k]), 0); enc.set_buffer(3, Some(&o.inp2[k]), 0);
            u32b(enc, 4, IN_ROWS as u32); u32b(enc, 5, IN_COLS as u32);
            u32b(enc, 6, (IN_COLS / 64) as u32);
        })?;
        dispatch_rec_on(&mut tcb, r, &r.state2, k, &r.out_seq2[k])?;
        tcb.dispatch_threads(SEQ_GATEUP, ((((GU_ROWS as u32) + 1) / 2) * 128, 1, 1), tg, |enc| {
            enc.set_buffer(0, Some(&w.gc), 0); enc.set_buffer(1, Some(&w.gs), 0);
            enc.set_buffer(2, Some(&w.gb), 0); enc.set_buffer(3, Some(&w.uc), 0);
            enc.set_buffer(4, Some(&w.us), 0); enc.set_buffer(5, Some(&w.ub), 0);
            enc.set_buffer(6, Some(&o.down[k]), 0);
            enc.set_buffer(7, Some(&o.gate2[k]), 0); enc.set_buffer(8, Some(&o.up2[k]), 0);
            u32b(enc, 9, GU_ROWS as u32); u32b(enc, 10, GU_COLS as u32);
        })?;
        tcb.dispatch_threads(SEQ_DOWN, ((((DN_ROWS as u32) + 1) / 2) * 128, 1, 1), tg, |enc| {
            enc.set_buffer(0, Some(&w.dc), 0); enc.set_buffer(1, Some(&w.ds), 0);
            enc.set_buffer(2, Some(&w.db), 0); enc.set_buffer(3, Some(x_dn), 0);
            enc.set_buffer(4, Some(&o.down2[k]), 0);
            u32b(enc, 5, DN_ROWS as u32); u32b(enc, 6, DN_COLS as u32); u32b(enc, 7, 64);
        })?;
    ...
    fn batched_all_positions(
        ...
        oi2: &metal::Buffer, og2: &metal::Buffer, ou2: &metal::Buffer, od2: &metal::Buffer,
    ) -> Result<(u64, u64), Box<dyn Error>> {
        let mut tcb = TokenCommandBuffer::new(ctx);
        tcb.begin_serial_group()?;
        let tg = (128u32, 1, 1);
        ...
        for k in 0..K {
            dispatch_rec_on(&mut tcb, r, &r.state, k, &r.out_bat[k])?;
        }
        ...
        // ---- LAYER 2 on the SAME chunk. `od` is out[row*K + k] with row over
        // 5120 hidden values; in_proj wants input[col*K + k] with col over 5120.
        // Same layout, so it is bound DIRECTLY. If that were wrong the outputs
        // below would diverge from the sequential arm rather than quietly differ.
        tcb.dispatch_threads(MP_INPROJ, g(IN_ROWS), tg, |enc| {
            enc.set_buffer(0, Some(&w.in_codes), 0); enc.set_buffer(1, Some(&w.in_scales), 0);
            enc.set_buffer(2, Some(od), 0); enc.set_buffer(3, Some(oi2), 0);
            u32b(enc, 4, IN_ROWS as u32); u32b(enc, 5, IN_COLS as u32);
            u32b(enc, 6, (IN_COLS / 64) as u32);
        })?;
        for k in 0..K {
            dispatch_rec_on(&mut tcb, r, &r.state2, k, &r.out_bat2[k])?;
        }
        tcb.dispatch_threads(MP_GATEUP, g(GU_ROWS), tg, |enc| {
            enc.set_buffer(0, Some(&w.gc), 0); enc.set_buffer(1, Some(&w.gs), 0);
            enc.set_buffer(2, Some(&w.gb), 0); enc.set_buffer(3, Some(&w.uc), 0);
            enc.set_buffer(4, Some(&w.us), 0); enc.set_buffer(5, Some(&w.ub), 0);
            enc.set_buffer(6, Some(od), 0);
            enc.set_buffer(7, Some(og2), 0); enc.set_buffer(8, Some(ou2), 0);
            u32b(enc, 9, GU_ROWS as u32); u32b(enc, 10, GU_COLS as u32);
        })?;
        tcb.dispatch_threads(MP_DOWN, g(DN_ROWS), tg, |enc| {
            enc.set_buffer(0, Some(&w.dc), 0); enc.set_buffer(1, Some(&w.ds), 0);
            enc.set_buffer(2, Some(&w.db), 0); enc.set_buffer(3, Some(xi_dn), 0);
            enc.set_buffer(4, Some(od2), 0);
            u32b(enc, 5, DN_ROWS as u32); u32b(enc, 6, DN_COLS as u32); u32b(enc, 7, 64);
        })?;
        tcb.end_serial_group()?;
        let d = tcb.dispatch_count() as u64;
    ...
    pub fn run() -> Result<(), Box<dyn Error>> {
        ...
        let o = Out {
            ...
            inp2: (0..K).map(|_| ctx.new_buffer_checked(IN_ROWS * 4)).collect::<Result<_, _>>()?,
            gate2: (0..K).map(|_| ctx.new_buffer_checked(GU_ROWS * 4)).collect::<Result<_, _>>()?,
            up2: (0..K).map(|_| ctx.new_buffer_checked(GU_ROWS * 4)).collect::<Result<_, _>>()?,
            down2: (0..K).map(|_| ctx.new_buffer_checked(DN_ROWS * 4)).collect::<Result<_, _>>()?,
        };
        ...
        let rec = Rec {
            ...
            out_seq2: (0..K).map(|_| ctx.new_buffer_checked(HEADS * VD * 4)).collect::<Result<_, _>>()?,
            out_bat2: (0..K).map(|_| ctx.new_buffer_checked(HEADS * VD * 4)).collect::<Result<_, _>>()?,
        };
        ...
        let oi2 = ctx.new_buffer_checked(IN_ROWS * K * 4)?;
        let og2 = ctx.new_buffer_checked(GU_ROWS * K * 4)?;
        let ou2 = ctx.new_buffer_checked(GU_ROWS * K * 4)?;
        let od2 = ctx.new_buffer_checked(DN_ROWS * K * 4)?;
        ...
        reset_state(&rec);
        let (_, bat_d) = batched_all_positions(&ctx, &w, &rec, &xi_in, &xi_gu, &xi_dn, &oi, &og, &ou, &od, &oi2, &og2, &ou2, &od2)?;
        let refs = [
            ("L2_in_proj", &o.inp2, &oi2, IN_ROWS),
            ("L2_gate", &o.gate2, &og2, GU_ROWS),
            ("L2_up", &o.up2, &ou2, GU_ROWS),
            ("L2_down", &o.down2, &od2, DN_ROWS),
        ...
        // stale state, this is where it shows.
        {
            let mut m = 0f64;
            for k in 0..(2 * K) {
                let (sq, bt) = if k < K { (&rec.out_seq, &rec.out_bat) } else { (&rec.out_seq2, &rec.out_bat2) };
                let k = k % K;
                let a = read_f32(&sq[k], HEADS * VD);
                let b = read_f32(&bt[k], HEADS * VD);
                for i in 0..a.len() {
        ...
        for (name, seq, bat, rows) in refs {
            ...
            for k in 0..K {
                ...
                for row in 0..rows {
                    ...
                    if d > mabs { mabs = d; }
                    let a = (s[row] as f64).abs();
                    if a > mag { mag = a; }
                    let rel = d / a.max(1e-6);
                    if rel > m { m = rel; at_ref = s[row] as f64; }
                }
            }
            if m > worst { worst = m; }
            let scaled = mabs / mag.max(1e-9);
            if scaled > worst_scaled { worst_scaled = scaled; }
            per_organ.push(json!({"organ": name, "max_rel_err": m, "max_abs_err": mabs,
                                  "err_over_tensor_magnitude": scaled,
                                  "largest_reference_magnitude": mag,
        ...
        if timing && valid {
            for rep in 0..(reps + 1) {
                ...
                reset_state(&rec);
                let b = batched_all_positions(&ctx, &w, &rec, &xi_in, &xi_gu, &xi_dn, &oi, &og, &ou, &od, &oi2, &og2, &ou2, &od2)?.0;
                if rep == 0 { continue; }
                sq.push(s); bq.push(b);
            }
        }
...
all K positions",
            ...
            "correctness": {
                "max_err_over_tensor_magnitude": worst_scaled,
                "max_raw_rel_err": worst,
                "tolerance_on_scaled": 1e-5,
                "valid": valid,
                "why_scaled": "raw relative error is meaningless at a near-zero reference; layer 2's worst raw rel of 2.169e-3 sits at a reference of -8.2e-5 while the same disagreement is 4.768e-7 on a 2.276 range. A WRONG LAYOUT would disagree by order 1, not by two ulps.",
                "per_organ": per_organ,
            },
