...
class TestGrokNotAvailable(_BridgeTest):
    ...
    def test_which_none_raises_specific_error(self):
        self.which_patch.stop()
        with patch("tools.haider.hcli.grok_bridge.shutil.which", return_value=None), patch(
            "tools.haider.hcli.grok_bridge.DEFAULT_GROK_RUN_HINT", self.ABSENT_HINT
        ):
    ...
    def test_installed_binary_is_found_without_being_on_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake.chmod(0o755)
            with patch("tools.haider.hcli.grok_bridge.shutil.which", return_value=None), patch(
                "tools.haider.hcli.grok_bridge.DEFAULT_GROK_RUN_HINT", fake
            ):
                os.environ.pop("GROK_RUN", None)
