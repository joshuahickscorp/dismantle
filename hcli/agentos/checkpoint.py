...
def _gate_summary(workspace: Path, repo_root: Path) -> Dict[str, Any]:
    ...
    flash_router_selection = _science_receipt(flash_router_selection_candidates, fields=("semantic_type", "compiler_stage", "source_identity", "config", "selection", "execution", "physical_graph", "noetic_ir", "native_selection_execution_observed", "whole_model_capability", "complete_token_runtime", "promotion_allowed", "claim_boundary", "next_action"))
    return {
        "recovery_gate": {
            **(recovery or {
                "status": "NOT_RUN",
    ...
    *,
    ...
    return {
        ...
        "next_actions": [
            ...
            "extend the source-independent Flash Noetic campaign across additional blocks and compose remaining organs; fill actual EBPW/token-ns fields only from native protected receipts",
        ],
