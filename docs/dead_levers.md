...
**This is the one canonical kill-ledger for hawking.** It merges (a) the
...
**Update 2026-06-03 — the broader "harness the CPU for decode tps" question (single-stream AND aggregate) empirically closed Type-1:** a dedicated investigation (`wf_b6d6913b`; `reports/research_final_push_2026_06_03.md`) confronted the full reframe — offloading matmul/decode work to CPU/AMX/ANE to raise tps, not just sampler overlap. **Single-stream:** dead by conservation of bandwidth on the one shared ~150 GB/s unified bus — decode time = bytes/token ÷ bandwidth, neither term changed by *who* reads; the GEMV runs ~56% of peak with **0.0 ms inter-dispatch idle** in production single-CB (`reports/dead_levers.md` GPU-saturation entry), so a concurrent CPU read *contends* (the AMX overlap collapses to 33% efficiency once the working set saturates LPDDR5, which decode does). **Aggregate:** the CPU decode path is **measured 0.06 dec_tps** (`HAWKING_FORCE_CPU=1`, Qwen2.5-3B, this session) — it dequantizes Q4_K→f32 every token = **~28.9 GB/token, 15× worse bytes/token than the GPU's ~1.9 GB**; adding 0.06-tps streams to a bus the GPU already saturates is a rounding error that steals more bandwidth than it adds. Web-confirmed: llama.cpp's `-ngl` CPU/GPU split is a memory-fit workaround, not a throughput win on unified memory; the GPU alone consumes ~85% of the bus (arxiv 2502.05317). **The live alternative for aggregate tps is GPU continuous batching** (read weights once, amortize across B sequences — `tools/bench/batch_ceiling.py` predicts ~3.5-5.6× at B=8), NOT CPU streams. The only Type-2 residue — a NEON-inline-Q4_K threaded CPU GEMV harvesting idle E-cores — is L-effort, contention-fragile, and strictly dominated by the GPU batched kernel; do not build before it.
...
**Evidence:** Three host-side hypotheses for the ~36 ms/token inter-dispatch "gap" all ruled out. (1) Q/K/V concurrent encoder (`begin/end_concurrent_group`, `HAWKING_QWEN_CONCURRENT_QKV=1`): bit-identical but paired only **+1.68%**, below the +5% ship gate. (2) PSO-transition batching: 200 dispatches with 199 PSO transitions runs **1.06×** vs identical-kernel — essentially free. (3) CPU encode is **0.51%** of wall ([[v230-icb-dead]], [[cpu-gpu-pipelining-audit]]). And `gpu_us` is **accurate** at production workload (host_wall/Σgpu = 1.03×), so the gap is NOT a measurement artifact — it is real GPU-side time. **Conclusion: every host-side per-dispatch lever (ICB, concurrent encoder, PSO batching, megakernel-for-dispatch-count) caps below the ship gate.** The real lever is kernel bandwidth efficiency (Bible Stage 2, ~41%→60%+ of peak), not dispatch overhead.
...
**Update 2026-06-03 — the "port llama.cpp's decode fusion" reframe is a NO-OP (already done) → Type-1 on this path:** the final-push deep-research (`reports/research_final_push_2026_06_03.md`, `wf_89411d2c`) proposed copying llama.cpp Metal **PR #16220** (NORM/RMS_NORM+MUL+ADD compile-time fusion-depth template) + **discussion #17621** (fuse MUL_MAT-after-ADD; reuse X in the gated activation σ(W_gate·X)⊙W_up·X) to collapse the ~180 Qwen dispatches/tok. A read-only code audit (`wf_5d69b392`, fusion spec) found **both are already implemented on the Qwen hot path by a different mechanism**: `add_rmsnorm_fused_tcb` already fuses residual-add + RMSNorm at both norm sites (qwen_dense.rs:4464, :4769) AND tail-hoists each layer's residual into the next layer's attn_norm (~73 dispatches/tok eliminated, comment at :3927-3929); FFN gate+up already fuses into one X-reused GEMV (`gemv_q4_k_v4_predec_pair_pinned_tcb`, :4522). The literal PR #16220 depth-template has **no F=2/F=3 work to absorb** — `add_rmsnorm_fused` already does the weight-multiply (common.metal:172) and residual-add (:157) *inside* the kernel; there is no standalone MUL/ADD node. So the specced fusion is **Type-1 dead on this path** (a measured property of the existing kernels, not impl cleverness). **Strategic consequence:** fusion is spent yet hawking is still ~0.62× llama.cpp at batch-1 decode → the 1.6× gap is NOT dispatch-count; it lives in inter-dispatch GPU idle / command-buffer structure / per-kernel cost, settle-able ONLY by the Metal System Trace diff (protocol in the research record).
**Type-2 reframe (alive, tiny):** fuse `silu_mul` INTO the ffn_down GEMV (a NEW `gemv_q4_k_v4_predec_silu_down` reading gate+up, applying SiLU⊙ inline; opt-in `HAWKING_QWEN_FUSE_SILU_DOWN=1`), removing 1 intermediate-sized dispatch/layer (36/tok). **Cheap oracle (named, in-hand):** build the kernel + an `atol=1e-3` parity test modeled on `add_rmsnorm_fused_q8_parity.rs`, then `tools/bench/ab_lever.sh --lever HAWKING_QWEN_FUSE_SILU_DOWN --blocks 3 --tokens 32`; GO only if it clears the +3% paired floor. Expected <+2% (likely noise — silu_mul is small vs the two GEMVs it sits between). NOT built (high surface area: ~6 ffn_down dtype branches each need the silu-folding twin) — squarely in the exhausted-micro-opt regime this entry records.
...
**Evidence:** `HAWKING_TCB_TRACE=cpu` measurement: per-token steady-state CPU encoding = 0.22 ms = 0.51% of wall. Ideal ICB ceiling +0.51% e2e; realistic +0.23%. Below the +5% ship gate.
...
**Pre-flight gate:** always run `HAWKING_TCB_TRACE=cpu` and check p50 weighted per-kernel encode sum vs Off-mode wall before proposing an ICB / megakernel / pipeline-replay lever.
...
**Evidence:** `gemm_q4_k_v4_predec_4r` (4 accumulator chains) was added on the theory that the decode GEMV underfills the GPU at 2 rows/simdgroup. But the homemade TCB trace that motivated it is split-CB-distorted and the §1 methodology gate rejects it ([[bible-execution-2026-05-30]]), so the "occupancy-starved" premise is unproven. The validated win is `_2r` (+6.2% bit-identical, default-on); `_4r` is bit-identical but **not adopted** and stays opt-in (`HAWKING_QWEN_PREDEC_4R=1`).
**2026-06-06 extension:** the same geometry idea was re-tested for fused gate/up pairs and Q6K SwiGLU. `PAIR_4R` vs old 2r pair: parity PASS, paired median `30.01` vs `30.28` dec_tps (`B/A=0.991`, `-0.9%`) and trace taxonomy now gates cleanly after adding missing kernel names. `PAIR_8R` vs explicit 4r: parity PASS, paired median `16.79` vs `29.62` (`B/A=0.567`, `-43.3%`) — 8 accumulator rows overrun the useful ILP/register-pressure envelope. `PAIR_3R` custom middle geometry vs 2r: parity PASS, gate PASS, paired median `31.48` vs `32.27` (`B/A=0.975`, `-2.5%`). `Q6K_SWIGLU_4R` vs default 2r: parity PASS, gate PASS, paired median `33.46` vs `33.93` (`B/A=0.986`, `-1.4%`). Keep these row-count variants opt-in only; do not default any 3r/4r/8r row-ILP geometry without a new Instruments oracle showing occupancy-bound behavior.
**2026-06-06 reframe/rescue:** the row-count idea was split into two variables: row geometry and scale-load style. `PAIR_2R_INLINE` keeps the proven 2-row geometry but drops the 64-float scale preload arrays in favor of inline scale reads. It is bit-identical to 2r; paired harness `e3_pair_2r_inline` is parity PASS + gate PASS with `36.40` vs `33.66` dec_tps (`B/A=1.081`, `+8.1%`), and a longer bench-only confirmation `e3_pair_2r_inline_confirm_64t` gives `36.51` vs `33.21` (`B/A=1.099`, `+9.9%`). This is a live clean-bench candidate behind `HAWKING_QWEN_PAIR_2R_INLINE=1`, not proof that more-row ILP should default.
**2026-06-06 fast-stack follow-up:** `PAIR_2R_INLINE` does **not** compound cleanly with f16 scale tables. E4 added `gemm_q4_k_v4_predec_pair_2r_inline_f16s`; parity and gate PASS, but the rerun is slower than the f16 pair baseline: `e4_pair_2r_inline_f16s` = `39.30` vs `41.64` dec_tps (`B/A=0.944`, `-5.6%`). E5 then split the route: B = `PAIR_2R_INLINE=1` with f32 scales on the pair path, C = f16-inline opt-in. B failed greedy-output parity and was slower (`39.96` vs `41.60`, `B/A=0.961`, `-3.9%`); C stayed exact but was also slower (`40.80`, `C/A=0.981`, `-1.9%`). Result: keep the f16-inline variant quarantined behind `HAWKING_QWEN_PAIR_2R_INLINE_F16S=1`; do **not** imply it from `PAIR_2R_INLINE=1`. The exact fast-stack default remains the proven `pair_f16s` path.
**2026-06-06 register-axis closure (F-track, "beyond-inline" register relief) — measured FLAT, axis exhausted:** the E3 win (inline scales) raised the question of whether *more* register relief on the pair GEMV could compound. First, **E3 itself was re-validated robustly** (64-tok × 7-trial, the noise-robust harness — 32-tok single runs proved contamination-unreliable, e.g. F2 below read +4.2% at 32-tok but −0.5% at 64-tok): `e3_verify_64t7` = `36.60` vs `33.34` dec_tps (`B/A=1.098`, **+9.8%**, parity PASS), all 7 B-trials above all 7 A-trials. **E3 is real and large — the standout default-flip candidate.** Then three beyond-inline customizations, ALL bit-identical (parity green incl. the production 11008×2048 shape), ALL measured FLAT:
- **F1 `pair_2r_inline_nox`** (strict pair: E3 inline + drop the `xl[8]` activation preload, read x per-pi — the f16x kill proved x≈0.026% of traffic so re-reading is ~0 bandwidth, frees ~6 regs): `B/A=0.997` (−0.3%) vs E3. The 64-float scale preload was the binding occupancy step; the 8 `xl` regs are below the next threshold.
- **F2 `pair_f16s_nox`** (headline f16 pair, keep half-scale preload, drop `xl`): 32-tok `B/A=1.042` (+4.2%, **contamination**) → robust 64-tok×7 `B/A=0.995` (−0.5%, FLAT). The f16s pair is 1-row/32-float-preload and **bandwidth-bound** (half-scales already cut the bytes), not occupancy-bound.
- **F3 `pair_f16s_halfreg`** (the novel "customize the dead lever" reframe of E4: keep scales in **half registers** packed 2/reg ≈16 regs vs 32, widen at FMA — a COALESCED preload unlike E4's lossy inline, + drop `xl`): `B/A=1.005` (+0.5%, FLAT). Either the compiler did not pack halves, or the reduction did not cross an occupancy threshold; the f16s pair is byte-bound regardless.
**Structural conclusion (the real value of this pass): the inline-scale axis is COMPLETE.** An audit of every hot strict-path GEMV found all of them *already* default to inline scales (0 preload arrays) — ffn_down `gemm_q4_k_v4_predec_4r_swiglu` (4r-inline, 4× silu amortization), o_proj `…_2r_add_rmsnorm` (4r-inline), qkv `gemm_q4k_predec_qkv_rope_append` (inline) — and the **gate+up pair is the SOLE remaining preload-default kernel**, which E3 closes. So there is no other inline-scale lever to harvest, and beyond-inline register relief (drop-xl, half-register packing) is tapped on both paths. The 6 new kernels (`pair_2r_inline_nox`, `pair_f16s_nox`, `pair_f16s_halfreg` + their wrappers/taxonomy/parity) stay opt-in behind `HAWKING_QWEN_PAIR_2R_INLINE_NOX` / `…_PAIR_F16S_NOX` / `…_PAIR_F16S_HALFREG` (bit-identical, harmless).
**Killing memory:** [[bible-execution-2026-05-30]], [[path-to-50-gap-corrected-2026-05-29]]
**Resurrection check:** re-evaluate more-row geometry ONLY after worklist 0.1 (xctrace profiling export) gives valid per-kernel occupancy/stall data. Adopt `_3r`/`_4r` only if profiling shows the predec GEMV is occupancy-limited (not bandwidth-limited) at 2 rows. Separately, `PAIR_2R_INLINE` is **DONE — SHIPPED default-on (2026-06-06).** CLEAN bench (agent quit, `wave_clean_bench.sh`): parity PASS, **B/A=1.096 (+9.6%)**, all 7 B-trials above all 7 A-trials; default flipped to on (opt-out `=0`), verified bit-identical (default == `=1` == `=0` in greedy tokens). This was the lone remaining inline-scale win and is now the default, consistent with every other hot strict GEMV already defaulting to inline scales. Do not rerun the f16-inline pair form unless an Instruments trace shows scale-table preload register pressure, not memory bandwidth, is the bottleneck in `pair_f16s`. **Do NOT re-spawn beyond-inline register relief on the pair (drop-xl/half-register packing) — F1/F2/F3 measured it FLAT 2026-06-06; the inline step is the entire occupancy win and the f16s pair is byte-bound.** A future register lever needs xctrace occupancy data (worklist 0.1) showing a register-bound stall AFTER inline, not a fresh guess.

---

## 🪦 Multiseq v4r-highB route (B=5..8 projections/FFN via v4r instead of v3w)

**Status:** NO-GO 2026-06-06, **clean-bench measured.** The wave-scout (`wf_ec6c7c51`) hypothesized — citing the closeout profile (`wf_60586084`, "v3w DRAM-latency-bound/under-occupied even at high B") — that routing B=5..8 multiseq projections + fused FFN-down through the barrier-free 16-rows/TG v4r_predec kernels (instead of the 8-rows/TG shmem-barrier v3w_predec) would lift the B=8 aggregate. Built as opt-in `HAWKING_QWEN_MULTISEQ_V4R_HIGHB` (bit-identical: parity-proven v4r==v3w at B=8 all shapes; greedy_token_only_parity green with the flag).
**Evidence:** CLEAN aggregate bench (`batch_aggregate_bench.sh`, agent quit, `wave_clean_bench.sh`), R1-ON (GPU Q4_K LM head), B=8 aggregate: **OFF = 62.78 tok/s → ON = 53.56 tok/s = −14.7%.** B=1 (28.31→28.66) and B=4 (45.09→44.65) were unchanged, so the regression is the flag (which only re-routes B>4), NOT thermal drift. The original routing comment was right: at B=8 the v4r kernel's strided per-element device-memory x-reads (B×256×4B/block) cost more than v3w's shmem-staged amortization. The closeout's "v3w under-occupied" reading does not flip the sign on the real model.
**Type:** Type-1 at B≤8 (a measured property of v4r's strided-x vs v3w's shmem-staging at high batch on M3 Pro). The flag stays default-OFF (harmless, bit-identical); do not flip it.
**Resurrection check:** do NOT re-route B=5..8 to v4r. A genuine high-B win needs a DIFFERENT kernel — either v3w with the 2-row-ILP/16-rows-TG geometry *added* (the closeout's literal proposal — a real rewrite, not the existing v4r) AND a clean B=8 A/B showing it beats today's v3w, OR paged-KV to push past B=8. The base continuous-batch stack (B=8 ≈ 48 tok/s clean, 5.0× scaling) is already shipped and is NOT in question here — only the v4r-for-highB re-route is dead.
...
**Update 2026-06-01 (paradigm Phase 1→2 boundary — reconciliation, does NOT resurrect):** the fresh 0.2 gap-anatomy (`paradigm_execution_log.md`) measured the predec GEMV at ~52% peak / a 1.55–1.63× llama gap and noted llama advertises `simdgroup matrix mul = true` + `use residency sets = true`. A dedicated adversarial reconciliation (`reports/scout_phase_2_gemv_reconciliation.md`) confirmed this does **NOT** reopen A5/A6/A10: (i) ggml-metal uses `mul_mv` (vec) at M=1 and `mul_mm` (MMA) only for batched/prefill — same split hawking made, so llama's MMA flag is **not** a decode-GEMV lever (cf. the "Q4_K batched MMA rows≤cols" Type-1 kill); (ii) `use residency sets` is Type-1 dead on unified memory (silicon #3); (iii) the one genuinely-distinct unscoped form — a half-precision **x** activation stream into the predec GEMV — is killed by byte arithmetic (x ≈ 0.03% of `_pair` traffic; the f16s bench `q4k_predec_f16s_bench.rs:125` already counts it). **The 1.55× gap is bytes (QTIP, Phase 4) + dispatch count (2.2), NOT a GEMV-kernel micro-opt.** `scout_phase_2_1_gemv.md` (which re-proposed A5-a/A5-b/A6) is **SUPERSEDED** by the reconciliation; do not execute its A5/A6/A10 kernel work. The cheap oracle for any future GEMV-BW claim is the existing `cargo test -p hawking-core --test q4k_predec_f16s_bench -- --nocapture` (GB/s readout); a form that doesn't beat the f16s readout at 11008×2048 is dead before integration.
...
**Reframe considered:** vectorized `uint` 4-byte load on the repacked layout (the only formulation that uses the contiguity). Dies twice: (a) NOT bit-identical — ~1 ULP FMA-recontraction drift (−71.67029 vs −71.67032) → fails the A10 hard gate; (b) no BW gain (48 GB/s < 58-64 GB/s baseline; 5-run sweep {−32,+16,+28,−23,+19}% = pure agent-app GPU contamination, no signal). No formulation gets a wider per-thread transaction while keeping BOTH bit-identity AND simdgroup coalescing.
...
|--:|----------|---------|------|----------|
| 8  | Q4_K simdgroup-MMA kernel | ✅ **LIVE → SHIPPED** | — | +10–20% batched Q4_K GEMM; shape-gated rows>cols, shipped `14434f5` (`HAWKING_QWEN_Q4K_MMA`) |
...
- **#15 int4 KV cache — HELD, quality-gated.** Per-channel int4 is the clear best scheme (cosine 0.998, rel_L2 ~0.06/head/layer) and a strong **long-context memory enabler** (1.2 GB→340 MB @32K, 3.5×) — but NOT a short-context tps lever (attention ~2.4% of decode) and per-layer error compounds over 36 layers. Hold pending a real-model perplexity/token gate (same rig as W4A8); ship as opt-in `--int4-kv` long-context mode only if it passes. Cross-ref `[[q8-kv-runtime-landed]]` (Q8 KV shipped; payoff only at long context).
  - **2026-06-07 — per-ROW int4 BUILT, end-to-end DISABLED (real-input finding).** A full per-row-symmetric int4 path was built + wired: kernels `kv_quant_int4_append` + `mha_decode_flash_int4kv` (mha.metal), arena buffers + `ensure_int4_kv`, and `forward_token_greedy_tcb` routing behind `HAWKING_QWEN_INT4_KV` (mutually excl. F16_KV/W4A8/FLASH_ATTN/BATCH_PREFILL). It passes a standalone cosine parity test at **0.996 on UNIFORM-random K/V** — but on the REAL model it emits incoherent output ("The The The…"). Root-caused by elimination: the **flash-f16kv control on the identical decode/append/prefill/offset path is coherent**, so routing is correct; the failure is the **per-ROW scheme** (one f16 scale per 128-elem token row) — post-RoPE K/V has per-channel outliers that dominate the row scale and round the rest to ~0. This is exactly why #15 specified **per-CHANNEL** scales. LESSON: a synthetic uniform-random parity test does NOT catch outlier-driven quant collapse; gate KV-quant schemes on REAL captured K/V. The flag now **fails loudly** (disabled; `HAWKING_QWEN_INT4_KV_EXPERIMENTAL=1` force-enables for redesign). Resurrection = re-do the append/decode kernels with **per-channel** scales (calibrated or running per-head_dim-channel), then re-gate on real K/V cosine + perplexity. Kernels/arena/routing are reusable scaffolding for that.
  - **2026-06-07 — per-CHANNEL redesign BUILT + validated on the outlier case.** Three new kernels (`kv_int4_calib_max` running-max fold over prefill → per-(layer,kvh,channel) f16 scale; `kv_quant_int4_append_pc`; `mha_decode_flash_int4kv_pc`) + 3 wrappers + 3 taxonomy lines + an OUTLIER-GATED parity test (`mha_decode_perchannel_int4kv_parity.rs`: synthesizes 5 channels at 10–50× and asserts cosine ≥0.98). Result: **cosine 0.982–0.993** across seq{64,128,129} on the adversarial synthetic — vs the per-row scheme's ~0.1 on the SAME input (the regression is fixed). GATE 1 (decode == CPU on the same int4) is exact. This validates the SCHEME; real captured K/V scores ~0.998 (#15) and a perplexity run on a free GPU is the ship gate. STILL TO DO (staged): wire the per-channel kernels into `forward_token_greedy_tcb` (calib over prefill → finalize /7 at the prefill→decode boundary → append_pc/decode_pc), replacing the per-row routing behind `HAWKING_QWEN_INT4_KV`. The hot-path wiring + perplexity are the remaining work; the novel numerics are now proven.
...
### 🪦 GPU-saturation / inter-dispatch-idle reclaim (push GPU-busy 76%→90%+) — NO-GO Type-1

**Status:** NO-GO 2026-06-02, **Type-1** for the saturation/idle-reclaim form. Closes the deep-research **#1 tps hypothesis** ("the 1.6× gap lives in command-buffer/GPU-saturation idle"; `research_next_levers_2026_06_02.md` TPS-#1).
**Evidence:** `tools/bench/gpu_saturation.sh` production gap decomposition (trace `/tmp/gpu_sat_trace_20260602T193249.json`, 22,176 samples). In **production** (one TokenCommandBuffer per token, ~616 kernels back-to-back) the measured **inter-token gap = 0.0 ms** and the GPU is continuously busy from first to last kernel within each token — there is **no intra-token inter-dispatch idle to reclaim**. The "GPU-busy ≈ 76% → ~24% reclaimable gap" figure that motivated the hypothesis came from the **0.2 `SplitCbGpu` trace, where each kernel runs in its OWN command buffer** — that mode inflates per-dispatch overhead by construction and does **not** reflect the single-CB production path. So "keep the GPU >90% busy / batch the command buffer / ICB-at-scale" is already satisfied in production; there is nothing to win there. (Cross-ref the already-dead host-side family: [[v230-icb-dead]], [[icb-production-scale]] +0.9% e2e, [[gpu-us-accuracy-verified-2026-05-24]] — CPU-encode is 0.51% of wall.)
**Note on the script's own auto-suggestion:** `gpu_saturation.sh` prints "next step: vectorized uint4 nibble load (A5)" — that heuristic is **stale**; A5 is a recorded Type-1 kill (GEMV micro-opt at the memory-model optimum). Ignore it.
**Reframe considered (ALIVE, Type-2):** the gap is real (llama.cpp = ~49 tps on this same M3 Pro vs hawking ~30.5) and the trace says it is **kernel-bound**, not idle — so the surviving formulation is "**llama's `mul_mv` Metal kernel runs the same Q4_K bytes in less GPU-time** (higher effective BW and/or fewer GPU-µs) via a technique hawking's pre-decoded GEMV doesn't match." This is a *different axis* from both the dead idle-reclaim form and the dead GEMV-micro-opt form (which tuned hawking's OWN kernel, not llama's algorithm).
**Why it stays parked until its oracle fires:** the named, in-hand oracle is a **Metal System Trace (Instruments) per-token timeline diff of llama.cpp vs hawking on M3 Pro** — does llama's GEMV sustain higher GiB/s per call, and by what mechanism (simdgroup_matrix tiling at M=1, a different weight layout, fused dequant)? Until that trace is run, the reframe is unproven on this hardware. **Caution / prior:** hawking already killed simdgroup-MMA at M=1 ([[qwen3b-dead-levers]], occupancy) and GEMV micro-opt (memory-model optimum) — so the reframe is *alive but adverse*; it may reduce to those dead forms once traced. Effort: medium (build/trace llama) then medium-high (port the technique), uncertain payoff.
**Resurrection check:** do NOT re-spawn "reclaim inter-dispatch idle / push GPU-busy / command-buffer batching / ICB-at-scale" as a tps lever — production is already ~100% intra-token GPU-busy (measured 2026-06-02, Type-1). The ONLY surviving tps reframe is the **llama-kernel-technique**, and it is admissible only **behind the Metal System Trace diff oracle** above — a measured llama-vs-hawking per-kernel GiB/s delta on M3 Pro. No trace → stays parked.
