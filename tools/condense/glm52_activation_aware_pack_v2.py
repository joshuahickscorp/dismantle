...
"""GLM-5.2 activation-aware pack v2 — feasibility + fake-data codec.

Opt-in, source-body-free successor path that corrects Generation B scientific
defects without starting a parent traversal or claiming a capable model.

Generation B defects this module encodes as law:

  * Retain the mean: uncentered SVD only (centered-only fitting forbidden).
  * Route-conditioned expert rows; empty/undersampled routes fail closed.
  * Real SwiGLU inputs for down_proj (Z = silu(X@Wg.T)*(X@Wu.T)).
  * Absolute organ-class floors; beats_null is diagnostic only.
  * Budget failure never reduces a rank or quality floor.
  * Exact once-per-basis-identity billing. Without a full route-population
    traffic map, the all-routed rank-128 uncertainty-bound ledger alone
    decides within_target_bpw. Rank-64 whole-population fit is a lower
    bound only (non-authorizing). Transfer-sharing is non-authorizing.

Does NOT modify glm52_activation_aware_pack.py defaults. Does NOT fetch or
traverse the 282 source weight bodies.

    python3.12 tools/condense/glm52_activation_aware_pack_v2.py selftest
    python3.12 tools/condense/glm52_activation_aware_pack_v2.py feasibility
"""
...
FORMAT_VERSION = 2
MAGIC = b"GLM52AA2"
HEADER_BYTES = 256  # richer metadata than v1's 64-byte header
SEED = 0xA17A7E2  # v2 seed; independent of v1
HIDDEN = aap.HIDDEN  # 6144
INTERMEDIATE = 2048
ORIGINAL_WEIGHT_COUNT = aap.ORIGINAL_WEIGHT_COUNT  # 753_329_940_480
SOURCE_PAYLOAD_BYTES = 1_506_659_919_872
...
PILOT_RECEIPT = REPO / "GLM52_BASIS_PILOT_RECEIPT.json"
CONTROLLER_RESEAL = REPO / "GLM52_BASIS_PILOT_CONTROLLER_RESEAL.json"
GEN_B_VERDICT = REPO / "GLM52_GENERATION_B_CAPABILITY_VERDICT.json"
DEFAULT_FEASIBILITY_JSON = REPO / "GLM52_V2_PROGRAM_FEASIBILITY.json"
DEFAULT_FEASIBILITY_MD = REPO / "GLM52_V2_PROGRAM_FEASIBILITY.md"

# Minimum route rows required for a fit (fail closed below this).
MIN_ROUTE_ROWS = 32

# ---------------------------------------------------------------------------
# Safety fences (bound into every receipt; never true in this module)
# ---------------------------------------------------------------------------
...
# Routed expert ranks used by whole-model ledgers (not traffic labels).
ROUTED_RANK_LOWER_BOUND = 64
ROUTED_RANK_UNCERTAINTY_BOUND = 128
SENSITIVITY_FRACTIONS: tuple[Fraction, ...] = (
    Fraction(0, 1),
    Fraction(1, 4),
    Fraction(1, 2),
    Fraction(3, 4),
...
# ---------------------------------------------------------------------------
# Preregistered heterogeneous candidate (bounded pilot; not whole-model capability)
# ---------------------------------------------------------------------------
# Floors are absolute. beats_null never admits a point below its floor.
# Static census classifies routed tensors neutrally (routed_gate/up/down).
# high_traffic / low_traffic keys remain pilot-evidence ranks only.
PREREGISTERED_PROGRAM: dict[str, dict[str, Any]] = {
    "routed_experts": {
        "organ_classes": (
            "routed_gate",
            "routed_up",
        ...
        "rank": ROUTED_RANK_LOWER_BOUND,  # default only; scenario may override
        "rank_uncertainty_bound": ROUTED_RANK_UNCERTAINTY_BOUND,
        "panel_floor_min_cosine": 0.85,
        "panel_floor_median_cosine": 0.96,
        "per_tensor_floor_cosine": None,
        "route_conditioned": True,
        "note": (
            "Neutral static census class for all routed experts. Rank is assigned "
            "by ledger scenario (64 lower-bound vs 128 uncertainty-bound), not by "
            "traffic. Traffic is not present in sealed source headers."
        ),
    },
    "high_traffic_routed_gate_up_down": {
        "organ_classes": (
        ...
        "route_conditioned": True,
        "note": (
            "Pilot evidence only (sealed five-shard high-traffic panel). Not used "
            "as a whole-model traffic classification in the static census."
        ),
    },
    "low_traffic_routed_diagnostics": {
        "organ_classes": (
        ...
        "basis_mode": "uncentered",
        "rank": 128,
        "panel_floor_min_cosine": None,
        "panel_floor_median_cosine": None,
        "per_tensor_floor_cosine": 0.91,
        "route_conditioned": True,
        "note": (
            "Pilot diagnostic only; sealed low-traffic panel needed rank 128 to "
            "clear the 0.91 per-tensor floor. Not a population traffic map."
        ),
    },
    "shared_mlp_gate_up_down": {
        "organ_classes": (
            "shared_mlp_gate",
            "shared_mlp_up",
            "shared_mlp_down",
        ...
        "rank": 256,
        "panel_floor_min_cosine": None,
        "panel_floor_median_cosine": 0.93,
        "per_tensor_floor_cosine": 0.91,
        "route_conditioned": False,
        "note": "Shared expert uses all real capsule rows + actual gate/up for down.",
    },
    "router_control": {
        "organ_classes": ("router_control",),
        "basis_mode": "uncentered",
        "rank": 128,
        "panel_floor_min_cosine": None,
        "panel_floor_median_cosine": None,
        "per_tensor_floor_cosine": 0.99,
        "route_conditioned": False,
        "note": "Router weight control; absolute floor 0.99.",
    },
    "attention_input_q_a_proj": {
        "organ_classes": ("attention_q_a_proj",),
        "basis_mode": "uncentered",
        "rank": 128,
        "panel_floor_min_cosine": None,
        "panel_floor_median_cosine": None,
        "per_tensor_floor_cosine": 0.91,
        "route_conditioned": False,
        "note": "Only supported attention input projection under current capsules.",
    },
}

# Classes that remain native / unvalidated in the feasibility census.
NATIVE_UNVALIDATED_REASONS: dict[str, str] = {
    "attention_o_proj": (
        "Capsules lack the 16384-wide attention intermediate; Gaussian input forbidden."
    ),
    "attention_other": "No bounded real-input pilot for this attention projection.",
    "global_embed_tokens": "embed_tokens not in five-shard pilot; remains native.",
    "global_lm_head": "lm_head not in five-shard pilot; remains native.",
    "norm": "Norms stay native (vector pass-through).",
    "router_bias": "Router bias/e_score stay native.",
    "dense_mlp": "Dense early layers (0-2) unvalidated under MoE pilot program.",
    "other": "No real-input pilot; billed native at source payload width.",
}
...
# ---------------------------------------------------------------------------
# Uncentered basis (mean-retaining; centered forbidden for v2 promotion)
# ---------------------------------------------------------------------------
@dataclass
class UncenteredBasis:
    """Orthonormal columns from uncentered SVD of real activation rows."""

    basis: np.ndarray  # [H, r] float32
...
def build_uncentered_basis(X_fit: np.ndarray, rank: int) -> UncenteredBasis:
    ...
    return UncenteredBasis(
        basis=B,
        singular_values=s[:r].astype(np.float32),
        rank=r,
        X_fit_mean=mu,
        mean_direction_unit=mdir,
        mode="uncentered",
    )
...
def mean_direction_retained(basis: UncenteredBasis, atol: float = 0.5) -> bool:
    ...
    if float(np.linalg.norm(basis.mean_direction_unit)) < 1e-8:
        return False
...
# ---------------------------------------------------------------------------
# Route conditioning (fail closed)
# ---------------------------------------------------------------------------
...
def select_route_rows(
    ...
    expert_id: int,
    *,
    min_rows: int = MIN_ROUTE_ROWS,
) -> np.ndarray:
    """Select real pre_router_hidden rows for expert E. Never falls back to all rows."""
    idx = route_row_indices(topk, expert_id)
    if idx.size == 0:
        raise RouteUndersampledError(
            f"expert {expert_id}: empty route selection; refuse all-row fallback"
        )
    if idx.size < int(min_rows):
        raise RouteUndersampledError(
            f"expert {expert_id}: only {idx.size} route rows < min_rows={min_rows}; "
            f"fail closed (no silent fallback)"
        )
    return np.asarray(X, dtype=np.float32)[idx]


# ---------------------------------------------------------------------------
# Real SwiGLU intermediate (down_proj input)
# ---------------------------------------------------------------------------
...
def swiglu_intermediate(
    ...
    if X.shape[1] != W_gate.shape[1]:
        raise V2Error(
            f"X width {X.shape[1]} != W_gate.in {W_gate.shape[1]}"
        )
    g = silu(X @ W_gate.T)
    u = X @ W_up.T
    return g * u
...
def assert_no_gaussian_promotion_path(source_text: str | None = None) -> None:
    ...
    banned = _re.compile(
        r"(?m)^\s*def\s+(gaussian_proxy|build_gaussian_activations|"
        r"proxy_activations|gaussian_probe_activations)\s*\("
    )
...
# ---------------------------------------------------------------------------
# Projection / reconstruction (input-side only for promotional scores)
# ---------------------------------------------------------------------------
def project_factors(W: np.ndarray, B: np.ndarray, side: str = "input") -> np.ndarray:
    if side != "input":
        raise V2Error(
            "v2 promotional path is input-side only "
            "(output-side down is a forbidden production negative control)"
        )
...
# ---------------------------------------------------------------------------
# Absolute floors + budget (never reduce floors for budget)
# ---------------------------------------------------------------------------
@dataclass
class FloorSpec:
...
def check_absolute_floors(
    ...
    floors: FloorSpec,
    *,
    beats_null_flags: Sequence[bool] | None = None,
) -> dict[str, Any]:
    ...
    if floors.per_tensor_min is not None:
        for i, c in enumerate(cos):
            if c < floors.per_tensor_min:
                ...
                bn = (
                    beats_null_flags[i]
                    if beats_null_flags is not None and i < len(beats_null_flags)
                    else None
                )
                failures.append(
                    f"tensor[{i}] cosine={c:.6f} < floor={floors.per_tensor_min}"
                    + (f" (beats_null={bn} ignored)" if bn is not None else "")
                )
    panel_min = min(cos)
    ...
    if floors.panel_min is not None and panel_min < floors.panel_min:
        panel_ok = False
        failures.append(
            f"panel min {panel_min:.6f} < floor {floors.panel_min}"
        )
    if floors.panel_median is not None and panel_med < floors.panel_median:
        panel_ok = False
        failures.append(
            f"panel median {panel_med:.6f} < floor {floors.panel_median}"
        )
    return {
        "ok": per_tensor_ok and panel_ok,
        "panel_min": panel_min,
        "panel_median": panel_med,
        "failures": failures,
        "beats_null_is_diagnostic_only": True,
    }


def select_program_or_native(
    *,
    cosine: float,
    beats_null: bool,
    floor: float,
    ...
    encoded_bytes: int,
    byte_budget_remaining: int | None,
) -> dict[str, Any]:
    ...
    if cosine < float(floor):
        return {
            "disposition": "native",
            "reason": "absolute_floor_failed",
            "cosine": cosine,
            "floor": float(floor),
            "beats_null": bool(beats_null),
            "beats_null_overrode_floor": False,
            "billed_bytes": int(source_payload_bytes),
            "billing": "source_payload_width",
        }
    # Floor cleared. Budget check cannot reduce rank/floor.
    if byte_budget_remaining is not None and encoded_bytes > byte_budget_remaining:
        raise BudgetFailure(
            f"encoded_bytes={encoded_bytes} exceeds remaining budget "
            f"{byte_budget_remaining}; refuse to lower rank or floor "
            f"(cosine={cosine} cleared floor={floor}; beats_null={beats_null} diagnostic only)"
        )
    return {
        "disposition": "activation_aware_v2",
        "reason": "absolute_floor_cleared",
        "cosine": cosine,
        "floor": float(floor),
        "beats_null": bool(beats_null),
        "billed_bytes": int(encoded_bytes),
        "billing": "coefficients_plus_shared_basis_share",
    }


# ---------------------------------------------------------------------------
# Basis identity + exact-once billing
# ---------------------------------------------------------------------------
def basis_identity(
    *,
    kind: str,
    ...
    if kind not in (
        "uncentered_hidden",
        "real_swiglu_input",
        "uncentered_attention_input",
        "uncentered_router_input",
    ):
...
class BasisLedger:
    ...
    def total_bytes(self) -> int:
        return (
            self.basis_bytes_total()
            + self.coefficient_bytes_total
            + self.tensor_header_bytes_total
            + self.native_bytes_total
            + self.packaging_bytes_total
        )

    def component_totals(self) -> dict[str, int]:
        return {
            "float16_basis_matrices": self.basis_bytes_total(),
            "float16_coefficient_matrices": int(self.coefficient_bytes_total),
            "tensor_headers_metadata": int(self.tensor_header_bytes_total),
            "native_source_payload": int(self.native_bytes_total),
            "packaging_alignment": int(self.packaging_bytes_total),
        }
    ...
    def as_dict(self, *, include_all_identities: bool = False) -> dict[str, Any]:
        ...
        by_kind: dict[str, dict[str, int]] = defaultdict(
            lambda: {"n_unique": 0, "refcount_sum": 0, "bytes": 0}
        )
        for b in ordered:
            k = str(b["kind"])
        ...
        out: dict[str, Any] = {
            "n_unique_bases": len(self.bases),
            "basis_identities_sorted_sha256": sha256_json(id_list),
            "basis_identity_examples": id_list[:12],
            "basis_by_kind": {k: dict(v) for k, v in sorted(by_kind.items())},
            "basis_refcount_sum": int(sum(b["refcount"] for b in self.bases.values())),
            "component_totals": comps,
            "total_bytes": total,
            "itemization_reconciles": sum(comps.values()) == total,
            "complete_bpw_exact": f"{bpw.numerator}/{bpw.denominator}",
            "complete_bpw_float": float(bpw),
            "n_encoded_tensors": self.n_encoded_tensors,
            "n_native_tensors": self.n_native_tensors,
            "note": (
                "Each physical basis is billed exactly once. Full identity strings "
                "are hashed (basis_identities_sorted_sha256); examples and by-kind "
                "counts are listed. Set include_all_identities for the full list."
            ),
        }
...
# ---------------------------------------------------------------------------
# Organ classification from tensor names
# ---------------------------------------------------------------------------
...
Q_A_RE = re.compile(r"model\.layers\.(\d+)\.self_attn\.q_a_proj\.weight$")
O_PROJ_RE = re.compile(r"model\.layers\.(\d+)\.self_attn\.o_proj\.weight$")
...
class TensorClass:
    ...
    layer: int | None
    expert_id: int | None
    projection: str | None  # gate_proj | up_proj | down_proj | q_a_proj | ...
    program_group: str | None  # key into PREREGISTERED_PROGRAM or None
    shape: list[int]
    payload_bytes: int
    n_weights: int
...
def classify_tensor(name: str, shape: Sequence[int], payload_bytes: int) -> TensorClass:
    ...
    if m:
        ...
        # Neutral static census: traffic is not present in sealed source headers.
        # Do not label high_traffic_* / low_traffic_* from headers alone.
        if proj == "gate_proj":
            organ = "routed_gate"
        elif proj == "up_proj":
            organ = "routed_up"
    ...
    if m:
        ...
        return TensorClass(
            ...
            expert_id=None,
            projection=proj,
            program_group="shared_mlp_gate_up_down",
            shape=shape_l,
            payload_bytes=pb,
            n_weights=nw,
        )
    ...
    if m:
        return TensorClass(
            name=name,
            organ_class="attention_q_a_proj",
            layer=int(m.group(1)),
            expert_id=None,
            projection="q_a_proj",
            program_group="attention_input_q_a_proj",
            shape=shape_l,
            payload_bytes=pb,
            n_weights=nw,
        )
    ...
    if name == "model.embed_tokens.weight" or name.endswith("embed_tokens.weight"):
        return TensorClass(
            name=name,
            organ_class="global_embed_tokens",
            layer=None,
            expert_id=None,
            projection=None,
            program_group=None,
    ...
    if "self_attn" in name:
        return TensorClass(
            name=name,
            organ_class="attention_other",
            layer=_layer_or_none(name),
            expert_id=None,
            projection=name.split(".")[-2] if name.endswith(".weight") else None,
            program_group=None,
            shape=shape_l,
            payload_bytes=pb,
            n_weights=nw,
...
def build_census(entries: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    reconcile = {
        "unique_tensor_names": len(names),
        "expected_unique_tensor_names": EXPECTED_TENSOR_COUNT,
        "unique_tensor_names_ok": len(names) == EXPECTED_TENSOR_COUNT,
        "original_weights": total_w,
        "expected_original_weights": ORIGINAL_WEIGHT_COUNT,
        "original_weights_ok": total_w == ORIGINAL_WEIGHT_COUNT,
        "source_payload_bytes": total_b,
        "expected_source_payload_bytes": SOURCE_PAYLOAD_BYTES,
        "source_payload_bytes_ok": total_b == SOURCE_PAYLOAD_BYTES,
    }
    if not all(
        [
            reconcile["unique_tensor_names_ok"],
            reconcile["original_weights_ok"],
            reconcile["source_payload_bytes_ok"],
        ]
    ):
    ...
    return {
        ...
        "organ_weights": dict(organ_weights),
        "organ_bytes": dict(organ_bytes),
        "n_tensors": len(classes),
    }


# ---------------------------------------------------------------------------
# Program ledgers (rank-64 lower bound, rank-128 uncertainty, transfer)
# ---------------------------------------------------------------------------
def _encode_tensor_into_ledger(
    tc: TensorClass,
    ledger: BasisLedger,
    *,
    scope: str,
    rank_override: int | None = None,
    basis_authorizing: bool = True,
) -> None:
    ...
    # Coefficients always billed per tensor.
    ledger.add_coefficients(rows, rank)

    # Basis identity depends on organ + projection.
    if tc.program_group == "routed_experts":
        ...
        if tc.projection in ("gate_proj", "up_proj"):
            ...
            ledger.add_basis(
                ...
                rank=rank,
                kind="uncentered_hidden",
                authorizing=bool(basis_authorizing) and scope == "target_local",
            )
        elif tc.projection == "down_proj":
            if scope == "target_local":
                bid = basis_identity(
                    kind="real_swiglu_input",
                    layer=tc.layer,
                    expert_id=tc.expert_id,
                    rank=rank,
                    scope=scope,
    ...
    if tc.program_group == "shared_mlp_gate_up_down":
        ...
        if tc.projection in ("gate_proj", "up_proj"):
            # Distinguish shared from routed expert bases.
    ...
    if tc.program_group == "attention_input_q_a_proj":
        ...
        ledger.add_basis(
            ...
            width=HIDDEN,
            rank=rank,
            kind="uncentered_attention_input",
            authorizing=True,
        )
        return

    # Unknown / pilot-only program groups are not census-encoded as programs.
    ledger.add_native(tc.payload_bytes)
...
def list_routed_experts(tensors: Sequence[TensorClass]) -> list[tuple[int, int]]:
    ...
    for tc in tensors:
        if (
            tc.program_group == "routed_experts"
            and tc.layer is not None
            and tc.expert_id is not None
        ):
            seen.add((int(tc.layer), int(tc.expert_id)))
    return sorted(seen)


def build_routed_rank_ledger(
    tensors: Sequence[TensorClass],
    *,
    routed_rank: int,
    scope: str = "target_local",
    basis_authorizing: bool = True,
    ...
    for tc in tensors:
        if tc.program_group == "routed_experts":
            _encode_tensor_into_ledger(
                tc,
                ledger,
                scope=scope,
                rank_override=int(routed_rank),
                basis_authorizing=basis_authorizing,
            )
        else:
            _encode_tensor_into_ledger(
                tc,
...
def build_routed_mixture_ledger(
    tensors: Sequence[TensorClass],
    *,
    rank128_experts: set[tuple[int, int]],
    rank64: int = ROUTED_RANK_LOWER_BOUND,
    rank128: int = ROUTED_RANK_UNCERTAINTY_BOUND,
) -> BasisLedger:
...
def build_all_routed_rank64_lower_bound_ledger(
    tensors: Sequence[TensorClass],
) -> BasisLedger:
    """All routed experts at rank 64 — optimistic/lower-bound only; non-authorizing."""
    return build_routed_rank_ledger(
        tensors,
        routed_rank=ROUTED_RANK_LOWER_BOUND,
        scope="target_local",
        basis_authorizing=False,
    )


def build_all_routed_rank128_uncertainty_bound_ledger(
    tensors: Sequence[TensorClass],
) -> BasisLedger:
    ...
    return build_routed_rank_ledger(
        tensors,
        routed_rank=ROUTED_RANK_UNCERTAINTY_BOUND,
        scope="target_local",
        basis_authorizing=True,
    )
...
def route_population_sensitivity(tensors: Sequence[TensorClass]) -> dict[str, Any]:
    ...
    for frac in SENSITIVITY_FRACTIONS:
        k = int(frac * n)  # exact for n divisible by 4 (GLM-5.2: 19456)
        ...
        if prev_total is not None and total < prev_total:
            raise V2Error(
                f"sensitivity non-monotonic at frac={frac}: {total} < {prev_total}"
            )
        prev_total = total
        sweep.append({
            "fraction_rank128": f"{frac.numerator}/{frac.denominator}",
            "fraction_rank128_float": float(frac),
            "n_rank128_experts": k,
            "n_rank64_experts": n - k,
            "total_bytes": total,
            "complete_bpw_exact": f"{bpw.numerator}/{bpw.denominator}",
            "complete_bpw_float": float(bpw),
            "within_target_bpw": bool(bpw <= TARGET_BPW),
            "itemization_reconciles": led.reconciles(),
        })

    # Exact max k under target: binary search on sorted prefix (monotonic).
    lo, hi = 0, n
    best = 0
    best_bytes = 0
    while lo <= hi:
        mid = (lo + hi) // 2
        led = build_routed_mixture_ledger(
            tensors, rank128_experts=set(experts[:mid])
        )
    ...
    return {
        "kind": "arithmetic_sensitivity",
        "not_traffic_classification": True,
        "selection_rule": "sorted_(layer, expert)_prefix",
        "expert_count_unit": (
            "one expert = gate/up/down triplet with shared hidden basis and "
            "separate real-SwiGLU-input basis"
        ),
        "n_routed_experts": n,
        "rank64": ROUTED_RANK_LOWER_BOUND,
        "rank128": ROUTED_RANK_UNCERTAINTY_BOUND,
        "target_bpw": f"{TARGET_BPW.numerator}/{TARGET_BPW.denominator}",
        "sweep": sweep,
        "max_rank128_experts_under_target_bpw": best,
        "max_rank128_fraction_under_target_bpw_exact": (
            f"{best_frac.numerator}/{best_frac.denominator}"
        ),
        "max_rank128_fraction_under_target_bpw_float": float(best_frac),
        "max_rank128_total_bytes": best_bytes,
        "monotonic_total_bytes": True,
        "note": (
            "Deterministic arithmetic sensitivity only. Does not classify experts "
            "by traffic and does not prove representation quality at either rank."
        ),
    }


# ---------------------------------------------------------------------------
# Fake codec / ABI proof
# ---------------------------------------------------------------------------
@dataclass
class V2TensorMeta:
    format_version: int
    ...
    layer: int
    expert_id: int | None
    projection_side: str  # always "input" for promotional v2
    basis_kind: str
    basis_identity: str
    rank: int
...
def _pack_meta(meta: V2TensorMeta) -> bytes:
    ...
    # layout:
    # 8 magic | u32 ver | u32 rows | u32 cols | u32 rank | i32 layer | i32 expert
    # | u8 side | u8 route | u8 kind_code | pad | identity utf-8 ...
    kind_codes = {
        "uncentered_hidden": 1,
        "real_swiglu_input": 2,
        "uncentered_attention_input": 3,
        "uncentered_router_input": 4,
    }
    kind_code = kind_codes.get(meta.basis_kind, 0)
    expert = -1 if meta.expert_id is None else int(meta.expert_id)
    # ver,rows,cols,rank: u32; layer,expert: i32 (expert=-1 if none); side,route: i8; kind: u8
    head = struct.pack(
        "<8sIIIIiibbB",
        MAGIC,
        int(meta.format_version),
        int(meta.rows),
        int(meta.cols),
        int(meta.rank),
        int(meta.layer),
        int(expert),
        1,  # input side
        1 if meta.route_conditioned else 0,
        kind_code,
    )
    # identity + organ + provenance as length-prefixed utf-8 blobs
    def _lp(s: str) -> bytes:
        b = s.encode("utf-8")
        if len(b) > 255:
            b = b[:255]
    ...
    body = (
        _lp(meta.basis_identity)
        + _lp(meta.organ_class)
        + _lp(meta.activation_provenance)
        + _lp(meta.basis_kind)
        + _lp(meta.projection_side)
    )
    raw = head + body
...
def _unpack_meta(blob: bytes) -> dict[str, Any]:
    ...
    ver, rows, cols, rank, layer, expert, side_code, route, kind_code = struct.unpack_from(
        "<IIIIiibbB", blob, 8
    )
    ...
    kind_names = {
        1: "uncentered_hidden",
        2: "real_swiglu_input",
        3: "uncentered_attention_input",
        4: "uncentered_router_input",
    }
    return {
        "format_version": ver,
        "rows": rows,
        ...
        "rank": rank,
        "layer": layer,
        "expert_id": None if expert < 0 else expert,
        "projection_side": proj_side or ("input" if side_code == 1 else "unknown"),
        "route_conditioned": bool(route),
        "basis_kind": basis_kind or kind_names.get(kind_code, "unknown"),
...
def encode_tensor_v2(
    ...
    # basis is stored separately under its identity; tensor payload is coeffs only
    # for the shared-basis ABI. For round-trip proof we append basis when the
    # caller uses a self-contained blob (see encode_self_contained).
    blob = head + body
    return blob


def encode_self_contained(
    W: np.ndarray,
    B: np.ndarray,
    meta: V2TensorMeta,
) -> bytes:
...
def build_down_basis_from_swiglu(
    X_route: np.ndarray,
    W_gate: np.ndarray,
    W_up: np.ndarray,
    rank: int,
    ...
    return {
        "Z": Z,
        "B_z": B_z,
        "X_route_sha256": _array_sha256(X_route),
        "Z_sha256": _array_sha256(Z),
        "B_z_sha256": _array_sha256(B_z),
        "route_row_count": int(X_route.shape[0]),
        "rank": int(rank),
    }


def verify_down_basis_witness(
    *,
    X_route: np.ndarray,
    W_gate: np.ndarray,
    W_up: np.ndarray,
    ...
    hash_b_direct = _array_sha256(np.asarray(B_z, dtype=np.float32)) == expected.get(
        "B_z_sha256"
    )
    ...
    return {
        ...
        "shape_ok": shape_ok,
        "numeric_close": close,
        "X_route_hash_match": hash_x,
        "Z_hash_match": hash_z,
        "B_z_hash_match": hash_b and hash_b_direct,
        "rebuilt_B_z_sha256": rebuilt["B_z_sha256"],
        "candidate_B_z_sha256": _array_sha256(np.asarray(B_z, dtype=np.float32)),
    }


def fake_gate_up_down_roundtrip(
    *,
    seed: int = SEED,
    layer: int = 5,
    expert_a: int = 11,
    ...
    # 1. Deterministic pre-router rows and synthetic top-k routes.
    X = (rng.standard_normal((n_tokens, hidden)) + 2.0).astype(np.float32)
    # Build topk so each expert gets a known, disjoint-enough row set.
    # Column 0 carries the primary expert assignment (alternating blocks).
    topk = np.zeros((n_tokens, 3), dtype=np.int32)
    half = n_tokens // 2
    topk[:half, 0] = expert_a
    topk[half:, 0] = expert_b
    # Fill remaining columns with distractors so selection is still route-based.
    topk[:, 1] = (topk[:, 0] + 3) % max(expert_b + 10, 200)
    topk[:, 2] = (topk[:, 0] + 7) % max(expert_b + 10, 200)
    # Ensure min_route_rows satisfied.
    if half < min_route_rows:
        raise V2Error(
            f"fake fixture n_tokens={n_tokens} yields {half} rows/expert "
            f"< min_route_rows={min_route_rows}"
        )

    # 2. Expert-specific route rows for hidden bases.
    ...
    # Matching fake weights for expert A (and gate for B).
    ...
    # 3–4. Real SwiGLU intermediate Z and down input basis B_z from Z (not random).
    ...
    id_h_a = basis_identity(
        kind="uncentered_hidden", layer=layer, expert_id=expert_a, rank=rank
    )
    id_h_b = basis_identity(
        kind="uncentered_hidden", layer=layer, expert_id=expert_b, rank=rank
    )
    id_z_a = basis_identity(
        kind="real_swiglu_input", layer=layer, expert_id=expert_a, rank=rank
    )
    id_z_b = basis_identity(
        kind="real_swiglu_input", layer=layer, expert_id=expert_b, rank=rank
    )
    ...
    def _meta(organ: str, eid: int, kind: str, bid: str, rows: int, cols: int) -> V2TensorMeta:
        return V2TensorMeta(
            ...
            basis_identity=bid,
            rank=rank,
            activation_provenance="fake_route_selected_swiglu",
            route_conditioned=True,
            rows=rows,
            cols=cols,
        )
    ...
    blobs = {
        "gate_a": encode_self_contained(
            W_gate_a,
            B_h_a,
            _meta("routed_gate", expert_a, "uncentered_hidden", id_h_a, intermediate, hidden),
        ),
        "up_a": encode_self_contained(
            W_up_a,
            B_h_a,  # shared with gate
            _meta("routed_up", expert_a, "uncentered_hidden", id_h_a, intermediate, hidden),
        ),
        "down_a": encode_self_contained(
            W_down_a,
            B_z_a,  # basis derived from real Z
            _meta("routed_down", expert_a, "real_swiglu_input", id_z_a, hidden, intermediate),
        ),
        "gate_b": encode_self_contained(
            W_gate_b,
            B_h_b,
            _meta("routed_gate", expert_b, "uncentered_hidden", id_h_b, intermediate, hidden),
        ),
    }

    dec = {k: decode_self_contained(v) for k, v in blobs.items()}

    # Gate/up share the same basis identity and decoded B (within float16).
    ...
    # Numeric reconstruction: float16 path is approximate; check coeff fidelity.
    for key, W, B_orig in (
        ("gate_a", W_gate_a, B_h_a),
        ("up_a", W_up_a, B_h_a),
        ("down_a", W_down_a, B_z_a),
        ("gate_b", W_gate_b, B_h_b),
    ):
        ...
        if err > 1e-2 * (float(np.max(np.abs(L_ref))) + 1e-6):
    ...
    # 6. Witnesses binding X_route, Z, B_z.
    witness_expected = {
        "X_route_sha256": down_a["X_route_sha256"],
        "Z_sha256": down_a["Z_sha256"],
        "B_z_sha256": down_a["B_z_sha256"],
    }
    positive = verify_down_basis_witness(
        X_route=X_route_a,
        W_gate=W_gate_a,
        W_up=W_up_a,
        B_z=B_z_a,
    ...
    # 7. Unrelated random basis cannot preserve the witness.
    B_unrelated = build_uncentered_basis(
        (rng.standard_normal((min_route_rows, intermediate)) + 0.5).astype(np.float32),
        rank,
    ).columns(rank)
    negative = verify_down_basis_witness(
        X_route=X_route_a,
        W_gate=W_gate_a,
        W_up=W_up_a,
        B_z=B_unrelated,
    ...
    # Hidden basis must come from route-selected rows (recompute check).
    B_h_a_re = build_uncentered_basis(X_route_a, rank).columns(rank)
    if not np.allclose(B_h_a, B_h_a_re, atol=1e-5):
    ...
    return {
        ...
        "gate_up_share": True,
        "experts_non_aliasing": True,
        "down_separate_swiglu_basis": True,
        "down_basis_from_real_swiglu": True,
        "hidden_basis_from_route_rows": True,
        "format_version": FORMAT_VERSION,
        "route_counts": {
            "expert_a": int(X_route_a.shape[0]),
            "expert_b": int(X_route_b.shape[0]),
            "n_tokens": int(n_tokens),
            "min_route_rows": int(min_route_rows),
        },
        "witnesses": {
            "expert_a": {
                "X_route_sha256": down_a["X_route_sha256"],
                "Z_sha256": down_a["Z_sha256"],
                "B_z_sha256": down_a["B_z_sha256"],
                "route_row_count": down_a["route_row_count"],
            },
            "expert_b": {
                "X_route_sha256": down_b["X_route_sha256"],
                "Z_sha256": down_b["Z_sha256"],
                "B_z_sha256": down_b["B_z_sha256"],
                "route_row_count": down_b["route_row_count"],
            },
            "positive_verify_ok": True,
            "unrelated_basis_fails_witness": True,
            "unrelated_B_z_sha256": _array_sha256(B_unrelated),
        },
        # Fixture handles for negative tests (small dims; deterministic under seed).
        "fixture_handles": {
            "layer": layer,
            "expert_a": expert_a,
            "expert_b": expert_b,
            ...
            "X_route_a_sha256": _array_sha256(X_route_a),
            "B_h_a_sha256": _array_sha256(B_h_a),
            "B_z_a_sha256": down_a["B_z_sha256"],
            "B_z_b_sha256": down_b["B_z_sha256"],
        },
    }


# ---------------------------------------------------------------------------
# Pilot checks (from sealed receipt; no re-measurement)
# ---------------------------------------------------------------------------
def pilot_checks_from_receipt(receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    uncentered_panels = (
        receipt.get("verdict", {}).get("panel_summaries", {}).get("uncentered", {})
    )
    ...
    # High-traffic rank 64 panel floors
    ...
    if ht and ht.get("min") is not None:
        ...
        cos = [ht["min"], ht["median"]]  # structural; full list not needed for seal check
        # Use reported panel stats directly
        ok = ht["min"] >= 0.85 and ht["median"] >= 0.96
        checks.append({
            "program": "high_traffic_routed_gate_up_down",
            "rank": 64,
            "panel": "promotion_grade_high_traffic_routed",
            "measured_min": ht["min"],
            "measured_median": ht["median"],
            "floor_min": 0.85,
            "floor_median": 0.96,
            "clears": ok,
        })

    # Low-traffic rank 128 per-tensor floor (panel min as proxy)
    lt = _panel_at("low_traffic_diagnostics", 128)
    if lt and lt.get("min") is not None:
        checks.append({
            "program": "low_traffic_routed_diagnostics",
            "rank": 128,
            "panel": "low_traffic_diagnostics",
            "measured_min": lt["min"],
            "measured_median": lt["median"],
            "per_tensor_floor": 0.91,
            "clears": lt["min"] >= 0.91,
        })
    ...
    if sh and sh.get("min") is not None:
        checks.append({
            "program": "shared_mlp_gate_up_down",
            "rank": 256,
            "panel": "shared_mlp",
            "measured_min": sh["min"],
            "measured_median": sh["median"],
            "per_tensor_floor": 0.91,
            "panel_median_floor": 0.93,
            "clears": sh["min"] >= 0.91 and sh["median"] >= 0.93,
        })

    # Per-tensor router / attention from tensor_results
        ...
        if organ == "router_control":
            ...
            if pt:
                checks.append({
                    ...
                    "rank": 128,
                    "name": t["name"],
                    "measured_cosine": pt["mean_row_cosine"],
                    "per_tensor_floor": 0.99,
                    "clears": pt["mean_row_cosine"] >= 0.99,
                })
        if organ == "attention_q":
            pt = next((p for p in arms if p.get("requested_rank") == 128), None)
            if pt:
                checks.append({
                    "program": "attention_input_q_a_proj",
                    "rank": 128,
                    "name": t["name"],
                    "measured_cosine": pt["mean_row_cosine"],
                    "per_tensor_floor": 0.91,
                    "clears": pt["mean_row_cosine"] >= 0.91,
                })

    uncentered_tied = receipt.get("verdict", {}).get(
        "uncentered_explicit_mean_numerically_tied", True
    )
    return {
        "pilot_schema": receipt.get("schema"),
        "pilot_revision": receipt.get("revision"),
        "basis_mode_chosen": "uncentered",
        "uncentered_explicit_mean_numerically_tied": uncentered_tied,
        "numerical_equivalence_tolerance": receipt.get(
            "numerical_equivalence_tolerance", 1e-4
        ),
        "centered_only_forbidden": True,
        "checks": checks,
        "all_candidate_checks_clear": all(c.get("clears", False) for c in checks),
        "gaussian_proxy_used_for_selection": receipt.get("safety", {}).get(
            "gaussian_proxy_used_for_selection", None
        ),
    }
...
def build_feasibility_receipt(
    *,
    ...
    # Top-level decision equals the all-rank-128 uncertainty bound only.
    within = bool(ub128_bpw <= TARGET_BPW)

    # Guard: never reduce ranks to force the uncertainty bound under budget.
    ranks_in_ub = {int(b["rank"]) for b in ub128.bases.values()}
    routed_ranks_ub = {
        int(b["rank"])
        for b in ub128.bases.values()
        if "|E" in str(b["identity"])  # routed expert identities
    }
    if routed_ranks_ub != {ROUTED_RANK_UNCERTAINTY_BOUND}:
        raise V2Error(
            f"uncertainty ledger routed ranks must be {{{ROUTED_RANK_UNCERTAINTY_BOUND}}}, "
            f"got {routed_ranks_ub}; refuse rank reduction to force budget"
        )
    ...
    # Unsupported / native classes summary
    unsupported = []
    for organ, reason in NATIVE_UNVALIDATED_REASONS.items():
        unsupported.append({
            "organ_class": organ,
            "n_tensors": census["organ_counts"].get(organ, 0),
            "n_weights": census["organ_weights"].get(organ, 0),
            "source_payload_bytes": census["organ_bytes"].get(organ, 0),
            "why": reason,
            "billing": "native_source_payload_width",
        })

    code_hashes = {
        "glm52_activation_aware_pack_v2_py_sha256": sha256_file(Path(__file__)),
        "glm52_activation_aware_pack_py_sha256": sha256_file(
            HERE / "glm52_activation_aware_pack.py"
        ),
    }
    ...
    source_hashes = {
        "GLM52_SOURCE_SHARD_HEADERS_sha256": sha256_file(headers_path),
    }
    if pilot_path.exists():
    ...
    if GEN_B_VERDICT.exists():
        source_hashes["GLM52_GENERATION_B_CAPABILITY_VERDICT_sha256"] = sha256_file(
            GEN_B_VERDICT
        )

    # Fake codec proof (deterministic)
    fake = fake_gate_up_down_roundtrip(seed=SEED)

    n_routed = len(list_routed_experts(tensors))
    # Static census must not use traffic labels for routed tensors.
    traffic_keys = [
        k for k in census["organ_counts"]
        if k.startswith("high_traffic_") or k.startswith("low_traffic_")
    ]
    if traffic_keys:
        raise V2Error(
            f"static census must not traffic-label routed organs; found {traffic_keys}"
        )
    ...
    receipt = {
        ...
        "revision": 1,
        "purpose": (
            "Opt-in feasibility + fake-data ABI for the next GLM-5.2 representation "
            "program. Corrects Generation B scientific defects without traversal or "
            "capability claims. Revision 1: route-population uncertainty + truthful "
            "fake SwiGLU down basis."
        ),
        "seed": SEED,
        "target_bpw": f"{TARGET_BPW.numerator}/{TARGET_BPW.denominator}",
        "preregistered_program": {
            k: {
                **{kk: (list(vv) if isinstance(vv, tuple) else vv) for kk, vv in v.items()}
            }
            for k, v in PREREGISTERED_PROGRAM.items()
        },
        "scientific_laws": {
            "basis_mode": "uncentered",
        ...
        "pilot_checks": pilot,
        "census": {
            "reconcile": census["reconcile"],
            "organ_counts": census["organ_counts"],
            "organ_weights": census["organ_weights"],
            "organ_bytes": census["organ_bytes"],
            "n_tensors": census["n_tensors"],
            "n_routed_experts": n_routed,
            "static_routed_classification": "routed_gate/up/down",
            "traffic_labels_from_headers": False,
        },
        "all_routed_rank64_lower_bound_ledger": {
            **lb64_d,
            "scope": "target_local",
            "routed_rank": ROUTED_RANK_LOWER_BOUND,
            "authorizing": False,
            ...
            "description": (
                "Target-local basis identities with all routed experts at rank 64. "
                "Optimistic/lower-bound byte scenario only — not conservative and not "
                "authorization-deciding. No full-model traffic map exists; the sealed "
                "low-traffic diagnostic needed rank 128 to clear its 0.91 floor."
            ),
            "within_target_bpw": bool(lb64_bpw <= TARGET_BPW),
        },
        "all_routed_rank128_uncertainty_bound_ledger": {
            **ub128_d,
            "scope": "target_local",
            "routed_rank": ROUTED_RANK_UNCERTAINTY_BOUND,
            "authorizing": True,
            ...
            "description": (
                "Target-local basis identities with all routed experts at rank 128 "
                "while shared MLP, router, and q_a retain preregistered ranks. This is "
                "the only ledger that may decide top-level within_target_bpw. It is a "
                "byte-feasibility uncertainty bound, not proof that rank 128 is "
                "quality-sufficient for every routed expert. Ranks are never reduced "
                "to force this total under budget."
            ),
            "within_target_bpw": within,
            "routed_ranks_present": sorted(routed_ranks_ub),
            "all_basis_ranks_present": sorted(ranks_in_ub),
        },
        "route_population_sensitivity": sensitivity,
        "transfer_sharing_scenario_ledger": {
            **xfer_d,
            "scope": "transfer_layer",
            "authorizing": False,
            "description": (
                "Non-authorizing scenario: one hidden basis and one SwiGLU basis per "
                "layer shared across experts. Cross-layer transfer remains unvalidated. "
                "Must not affect any top-level decision."
            ),
            "within_target_bpw_not_applicable": True,
            "note": "Do not use this total to authorize a traversal or claim capability.",
        },
        "within_target_bpw": within,
        ...
        "next_safe_action": (
            "Route-population measurement is required before any full traversal. "
            "A passing rank-mixture budget alone would still not prove representation "
            "capability. Keep v2 opt-in; never lower absolute floors or ranks to force "
            "the uncertainty bound under target BPW. Do not start a full traversal "
            "from this receipt alone."
        ),
    }
    # Deterministic content hash over the body excluding the hash field itself.
...
def feasibility_markdown(receipt: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "Revision 1: the rank-64 whole-population total is a **lower bound only** "
        "(not conservative). Top-level `within_target_bpw` is decided solely by the "
        "all-routed rank-128 **uncertainty bound** (byte feasibility, not quality proof).",
        "",
    ...
    lines += [
        ...
        "",
        f"- `full_route_population_classified`: "
        f"**{receipt['full_route_population_classified']}**",
        ...
        f"- `rank64_population_fit_is_lower_bound_only`: "
        f"**{receipt['rank64_population_fit_is_lower_bound_only']}**",
        f"- `full_traversal_authorized`: **{receipt['full_traversal_authorized']}**",
        f"- Routed experts (static census): "
        f"**{receipt['census'].get('n_routed_experts', 'n/a'):,}**",
        f"- Static routed classification: "
        f"`{receipt['census'].get('static_routed_classification')}` "
        "(not traffic labels)",
        "",
        "## Census (sealed headers)",
        "",
        f"- Unique tensors: **{rec['unique_tensor_names']}** "
        f"(expected {rec['expected_unique_tensor_names']}) "
        f"{'OK' if rec['unique_tensor_names_ok'] else 'FAIL'}",
        f"- Original weights: **{rec['original_weights']:,}** "
        f"{'OK' if rec['original_weights_ok'] else 'FAIL'}",
        f"- Source payload bytes: **{rec['source_payload_bytes']:,}** "
        f"{'OK' if rec['source_payload_bytes_ok'] else 'FAIL'}",
        "",
        ...
        "",
        "Not conservative. Optimistic whole-population scenario with every routed "
        "expert at rank 64.",
        "",
        f"- Unique bases: **{lb64['n_unique_bases']:,}**",
        f"- Total bytes: **{lb64['total_bytes']:,}**",
        f"- Complete BPW: **{lb64['complete_bpw_exact']}** "
        f"({lb64['complete_bpw_float']:.6f})",
        f"- `authorizing`: **{lb64['authorizing']}**",
        f"- Scenario within target (informational): **{lb64['within_target_bpw']}**",
        f"- Itemization reconciles: **{lb64['itemization_reconciles']}**",
        "",
        "Component totals:",
        "",
    ]
    ...
    lines += [
        ...
        "",
        "Byte-feasibility uncertainty bound for the whole routed population at rank "
        "128. **Not** proof that rank 128 is quality-sufficient for every expert. "
        "This total alone decides top-level `within_target_bpw`.",
        "",
        f"- Unique bases: **{ub128['n_unique_bases']:,}**",
        f"- Total bytes: **{ub128['total_bytes']:,}**",
        f"- Complete BPW: **{ub128['complete_bpw_exact']}** "
        f"({ub128['complete_bpw_float']:.6f})",
        f"- Target: **{receipt['target_bpw']}**",
        f"- `within_target_bpw` (top-level): **{receipt['within_target_bpw']}**",
        f"- `authorizing`: **{ub128['authorizing']}**",
        f"- Itemization reconciles: **{ub128['itemization_reconciles']}**",
        "",
        "Component totals:",
        "",
    ]
    ...
    lines += [
        ...
        "",
        f"- Selection: `{sens['selection_rule']}`",
        f"- Expert unit: {sens['expert_count_unit']}",
        f"- N routed experts: **{sens['n_routed_experts']:,}**",
        f"- Max rank-128 experts under target: "
        f"**{sens['max_rank128_experts_under_target_bpw']:,}** "
        f"({sens['max_rank128_fraction_under_target_bpw_exact']})",
        "",
    ...
    for p in sens["sweep"]:
        lines.append(
            f"| {p['fraction_rank128']} | {p['n_rank128_experts']:,} | "
            f"{p['total_bytes']:,} | {p['complete_bpw_float']:.6f} | "
            f"{p['within_target_bpw']} |"
        )
    lines += [
        "",
        "## Transfer-sharing scenario (NON-AUTHORIZING)",
        "",
        f"- Unique bases: **{xfer['n_unique_bases']:,}**",
        f"- Total bytes: **{xfer['total_bytes']:,}**",
        f"- Complete BPW (informational): **{xfer['complete_bpw_exact']}**",
    ...
    for c in pc.get("checks", []):
        lines.append(
            f"- `{c.get('program')}` rank {c.get('rank')}: "
            f"clears={c.get('clears')} detail={{{', '.join(f'{k}={v}' for k,v in c.items() if k not in ('program','rank'))}}}"
        )
    lines += [
        "",
    ...
    for u in receipt["unsupported_classes"]:
        if u["n_tensors"]:
            lines.append(
                f"- `{u['organ_class']}`: n={u['n_tensors']}, "
                f"bytes={u['source_payload_bytes']:,} — {u['why']}"
            )
    lines += [
        "",
...
def write_feasibility(
    *,
    out_json: Path = DEFAULT_FEASIBILITY_JSON,
    out_md: Path = DEFAULT_FEASIBILITY_MD,
) -> dict[str, Any]:
...
def selftest() -> int:
    ...
    # 1. Uncentered retains mean; differs from centered.
    h, n, r = 48, 200, 8
    ...
    # 4. Absolute floors beat beats_null.
    floors = FloorSpec(per_tensor_min=0.90)
    # cosine 0.50 beats a null of 0.40 but fails absolute floor
    res = check_absolute_floors([0.50], floors, beats_null_flags=[True])
    assert res["ok"] is False
    sel = select_program_or_native(
        cosine=0.50,
        beats_null=True,
        floor=0.90,
    ...
    # 5. Budget failure never reduces floor.
    try:
        select_program_or_native(
            cosine=0.95,
            beats_null=True,
            floor=0.90,
            source_payload_bytes=1000,
    ...
    # 6. Fake codec round-trip with real SwiGLU down basis.
    ...
    # 8. No Gaussian promotion path.
    assert_no_gaussian_promotion_path()

    # 9. Safety fences all false.
    assert all(v is False for v in SAFETY_FENCES.values())

    # 10. Census + feasibility (may take a few seconds on full headers).
    if SOURCE_HEADERS.exists():
        ...
        assert receipt["within_target_bpw"] == (
            receipt["all_routed_rank128_uncertainty_bound_ledger"]["within_target_bpw"]
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        nargs="?",
        default="selftest",
        choices=("selftest", "feasibility"),
        help="selftest (default) or feasibility",
    )
    ap.add_argument("--out-json", type=Path, default=DEFAULT_FEASIBILITY_JSON)
    ap.add_argument("--out-md", type=Path, default=DEFAULT_FEASIBILITY_MD)
    args = ap.parse_args(argv)

    if args.command == "selftest":
        return selftest()
    if args.command == "feasibility":
        receipt = write_feasibility(out_json=args.out_json, out_md=args.out_md)
            ...
            json.dumps(
                {
                    ...
                    "within_target_bpw": receipt["within_target_bpw"],
                    "rank128_uncertainty_bpw_exact": ub["complete_bpw_exact"],
                    "rank128_uncertainty_total_bytes": ub["total_bytes"],
                    "rank64_lower_bound_bpw_exact": lb["complete_bpw_exact"],
                    "rank64_lower_bound_total_bytes": lb["total_bytes"],
                    "rank64_authorizing": lb["authorizing"],
                    "full_route_population_classified": receipt[
                        "full_route_population_classified"
                    ],
                    "max_rank128_experts_under_target": receipt[
                        "route_population_sensitivity"
                    ]["max_rank128_experts_under_target_bpw"],
