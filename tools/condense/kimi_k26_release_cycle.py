...
# Worktree root (this checkout). Reports and campaign artifacts resolve here.
_A1_WORKTREE = _A1_CONDENSE.parents[1]


def _a1_primary_repo_root(condense_dir: _Path_a1) -> _Path_a1:
    """Primary worktree that owns the sealed transfer venv.

    Linked worktrees do not carry `.venv/glm52`. The transfer runtime binding is
    a machine-grounded, no-follow seal of that venv (absolute shebang + sha256),
    so verification must bind the primary checkout where the venv lives.
    """
    worktree = condense_dir.parents[1]
    ...
    if gitfile.is_file():
        ...
        if text.startswith("gitdir:"):
            gitdir = _Path_a1(text.split(":", 1)[1].strip()).resolve()
...
_A1_PRIMARY = _a1_primary_repo_root(_A1_CONDENSE)
...
REPO_ROOT = _A1_WORKTREE
# Sealed transfer runtime lives on the primary worktree (see _a1_primary_repo_root).
_TRANSFER_REPO = _A1_PRIMARY if (_A1_PRIMARY / ".venv/glm52").is_dir() else REPO_ROOT
...
HF_CLI = _TRANSFER_REPO / ".venv/glm52/bin/hf"
TRANSFER_VENV_ROOT = _TRANSFER_REPO / ".venv/glm52"
...
def build_transfer_runtime_binding() -> dict[str, Any]:
    ...
    # Engine-level launcher-node invariant (symlink / hard-link / wrong mode).
    try:
        from tools.condense.engine.seal_integrity import (  # noqa: WPS433
            SealIntegrityError as _EngineSealError,
            inspect_launcher_node as _engine_inspect_launcher,
        )
        _engine_inspect_launcher(
            HF_CLI,
            label="hf transfer launcher",
            expected_mode=0o755,
            require_single_hard_link=True,
            refuse_symlink=True,
        )
    except ImportError:
        pass
    except Exception as exc:  # engine guard failures surface as release errors
        if exc.__class__.__name__ == "SealIntegrityError" or "regular" in str(exc) \
                or "hard-link" in str(exc) or "mode" in str(exc) or "symlink" in str(exc):
            _fail(str(exc))
        raise
...
def verify_transfer_runtime_binding(value: dict[str, Any]) -> dict[str, Any]:
    # Engine-level reseal/substitution rejection — same guarantee for every campaign.
    try:
        from tools.condense.engine.seal_integrity import (  # noqa: WPS433
            SealIntegrityError as _EngineSealError,
            reject_resealed_substitution as _engine_reject_reseal,
        )
        return _engine_reject_reseal(
            value,
            build_transfer_runtime_binding,
            label="Kimi transfer runtime binding",
            match="exact deterministic runtime",
        )
    except ImportError:
        pass
    except Exception as exc:
        if exc.__class__.__name__ == "SealIntegrityError":
            _fail(str(exc))
        raise
...
def build_preflight(
    ...
    *,
    ...
    # Engine contract: pure preflight never shells out.
    try:
        from tools.condense.engine.seal_integrity import (  # noqa: WPS433
            preflight_must_not_use_subprocess as _engine_no_subprocess,
        )
        _engine_no_subprocess(subprocess_used=False, label="phase-1 preflight")
    except ImportError:
        pass
    layout_evidence = validate_layout(layout, mop_root=mop_root, shared_xet=shared_xet)
    manifest = verify_manifest(manifest_path)
