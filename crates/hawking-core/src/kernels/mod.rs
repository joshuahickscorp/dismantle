...
pub fn rope_inplace_scaled(x: &mut [f32], pos: u32, base: f32, scaling: Option<Llama3RopeScaling>) {
    ...
    for i in 0..half {
        ...
        let freq_eff = if wavelen < high_wavelen {
            ...
            let smooth = (s.original_max_position_embeddings as f32 / wavelen - s.low_freq_factor)
...
mod metal_dispatch {
    use crate::metal::{
        ArgLayout, CommandBatch, DecodeArena, KernelArgBuffer, MetalContext, PinnedBuffer,
        TokenCommandBuffer,
    };
    ...
    /// Shared encode/validate/bind/dispatch for the batched Q4_K GEMM family
    /// (v3w / v3 / v2 / v3w_mma). Public wrappers retain exact signatures and
    /// error-string identity; only the kernel name, batch window, TG geometry,
    /// shmem policy, and v2 OOB check differ.
    #[derive(Copy, Clone)]
    ...
    fn gemm_q4_k_m_batched_pinned_tcb(
        mode: BatchedQ4kGemmMode,
        tcb: &mut TokenCommandBuffer<'_>,
        model_buf: &PinnedBuffer,
        w_offset: usize,
        w_byte_size: usize,
        ...
        let (kernel, batch_max, tg, fixed_shmem, check_oob) = match mode {
            BatchedQ4kGemmMode::V3w => ("gemm_q4_k_m_batched_v3w", 8u32, 256u32, None, false),
        ...
        if !(1..=batch_max as usize).contains(&batch) {
            return Err(Error::Kernel(format!(
                "{kernel}_pinned_tcb supports batch in 1..={batch_max}; got {batch}"
            )));
        }
        let blocks_per_row = cols / 256;
        let expected_bytes = rows
        ...
        if check_oob && w_offset + w_byte_size > model_buf.length() as usize {
            return Err(Error::Kernel(format!(
        ...
        let shmem_bytes = match fixed_shmem {
            Some(n) => (n * std::mem::size_of::<f32>()) as u64,
            None if matches!(mode, BatchedQ4kGemmMode::V2) => 0,
            None => (batch * 256 * std::mem::size_of::<f32>()) as u64,
        };
        let mut ab =
    ...
    pub fn gemv_q4_k_v4_predec_pinned_tcb(
        ...
        let use_2r = {
            ...
            *E.get_or_init(|| {
                std::env::var_os("HAWKING_QWEN_PREDEC_2R")
                    .map(|v| v != "0")
                    .unwrap_or(true)
            })
        };
        ...
        let use_4r = {
            static E: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
            *E.get_or_init(|| {
                std::env::var_os("HAWKING_QWEN_PREDEC_4R")
                    .map(|v| v != "0")
                    .unwrap_or(false)
            })
        };
    ...
    pub fn gemv_q4_k_v4_predec_2r_add_rmsnorm_tcb(
        ...
        let use_4r = *USE_4R.get_or_init(|| {
            std::env::var_os("HAWKING_QWEN_OPROJ_4R")
                .map(|v| v != "0")
                .unwrap_or(false)
        });
        if use_4r {
    ...
    pub fn gemv_q4_k_v4_predec_swiglu_pinned_tcb(
        ...
        let use_4r = {
            static E: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
            *E.get_or_init(|| {
                !std::env::var_os("HAWKING_QWEN_SWIGLU_4R")
                    .map(|v| v == "0")
                    .unwrap_or(false)
            })
        };
    ...
    pub fn gemv_q6_k_swiglu_pinned_tcb(
        ...
        let use_4r = {
            ...
            *E4R.get_or_init(|| {
                std::env::var_os("HAWKING_QWEN_Q6K_SWIGLU_4R")
                    .map(|v| v != "0")
                    .unwrap_or(false)
            })
        };
        ...
        let use_2r = !use_4r && {
            static E: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
            *E.get_or_init(|| {
                !std::env::var_os("HAWKING_QWEN_Q6K_SWIGLU_2R")
                    .map(|v| v == "0")
                    .unwrap_or(false)
            })
        };
    ...
    /// Shared body for the Q4K predec QKV rope-append quartet (2r/4r × f32/f16s).
    /// Differs only by pipeline key and 2r vs 4r threadgroup geometry (+ 4r row multiple check).
    fn gemv_q4k_predec_qkv_rope_append_body(
        tcb: &mut TokenCommandBuffer<'_>,
        kernel: &str,
        rows4: bool,
        ...
        if rows4 && (q_rows % 4 != 0 || kv_rows % 4 != 0) {
            return Err(Error::Kernel(format!(
    ...
    pub fn gemv_q4k_predec_qkv_rope_append_pinned_tcb(
        ...
        gemv_q4k_predec_qkv_rope_append_body(
            tcb,
            "gemm_q4k_predec_qkv_rope_append",
            false,
            model_buf,
            q_offset,
            q_byte_size,
    ...
    pub fn gemv_q4k_predec_qkv_rope_append_f16s_pinned_tcb(
        ...
        gemv_q4k_predec_qkv_rope_append_body(
            tcb,
            "gemm_q4k_predec_qkv_rope_append_f16s",
            false,
            model_buf,
            q_offset,
            q_byte_size,
    ...
    pub fn gemv_q4k_predec_qkv_rope_append_4r_f16s_pinned_tcb(
        ...
        gemv_q4k_predec_qkv_rope_append_body(
            tcb,
            "gemm_q4k_predec_qkv_rope_append_4r_f16s",
            true,
            model_buf,
            q_offset,
            q_byte_size,
    ...
    pub fn dispatch_gemv_q4_k_m_simd_pair_silu_batched(
        ...
        if w_gate_bytes.len() != expected_bytes {
            return Err(Error::Kernel(format!(
                "pair+silu w_gate bytes: got {} expected {}",
                w_gate_bytes.len(),
                expected_bytes
            )));
        }
        if w_up_bytes.len() != expected_bytes {
            return Err(Error::Kernel(format!(
                "pair+silu w_up bytes: got {} expected {}",
                w_up_bytes.len(),
                expected_bytes
            )));
        }
    ...
    /// Shared MLA / flash-attention decode host dispatch.
    /// Modes differ by pipeline key, c_kv element type/layout check, error label,
    /// and flash threadgroup / shmem geometry.
    #[derive(Copy, Clone)]
    enum MlaDecodeMode {
        F32,
        Q8Kv,
    ...
    fn mla_decode_dispatch(
        ...
        let label = match mode {
            MlaDecodeMode::F32 => "mla_decode_metal",
            MlaDecodeMode::Q8Kv => "mla_decode_q8kv_metal",
            MlaDecodeMode::Flash => "flash_attn_decode_metal",
        };
        if matches!(mode, MlaDecodeMode::Q8Kv) && kv_lora_rank % 32 != 0 {
            return Err(Error::Kernel(format!(
                "{label}: kv_lora_rank {kv_lora_rank} not multiple of 32"
            )));
        }
        let q_head_dim = qk_nope_head_dim + qk_rope_head_dim;
        if q.len() != n_heads * q_head_dim {
            return Err(Error::Kernel(format!(
                "{label}: q.len={} expected {}",
                q.len(),
                n_heads * q_head_dim
            )));
        }
        match mode {
            MlaDecodeMode::Q8Kv => {
                ...
                if c_kv_bytes.len() < seq_len * row_bytes {
                    return Err(Error::Kernel(format!(
                        "{label}: c_kv_q8 len={} need at least {}",
                        c_kv_bytes.len(),
                        seq_len * row_bytes
                    )));
                }
            ...
            _ => {
                let expected = seq_len * kv_lora_rank * std::mem::size_of::<f32>();
                if c_kv_bytes.len() != expected {
                    return Err(Error::Kernel(format!(
                        "{label}: c_kv.len={} expected {}",
                        c_kv_bytes.len() / std::mem::size_of::<f32>(),
        ...
        if kv_b_proj.length() < expected_kv_b {
            return Err(Error::Kernel(format!(
                "{label}: kv_b_proj buffer too small: got {} expected {}",
                kv_b_proj.length(),
                expected_kv_b
            )));
        }
        if out.len() != n_heads * v_head_dim {
            return Err(Error::Kernel(format!(
                "{label}: out.len={} expected {}",
                out.len(),
                n_heads * v_head_dim
            )));
        }
        if seq_len == 0 {
            return Err(Error::Kernel(format!("{label}: seq_len must be >= 1")));
        }

        let q_buf = ctx.new_buffer_with_bytes(bytemuck::cast_slice::<f32, u8>(q));
        let c_kv_buf = ctx.new_buffer_with_bytes(c_kv_bytes);
        ...
        match mode {
            ...
            MlaDecodeMode::F32 | MlaDecodeMode::Q8Kv => {
                let kernel = if matches!(mode, MlaDecodeMode::Q8Kv) {
    ...
    pub fn mla_decode_metal(
        ...
        mla_decode_dispatch(
            ...
            ctx,
            q,
            bytemuck::cast_slice::<f32, u8>(c_kv),
            k_pe,
            kv_b_proj,
            n_heads,
            qk_nope_head_dim,
    ...
    /// Shared pinned Q4_K GEMV host path parameterized by pipeline key and TG geometry.
    /// Supersedes the near-identical simdmat / v3_8r / v3_dual / v3_llama private bodies.
    fn dispatch_q4_k_m_pinned_geometry(
        ctx: &MetalContext,
        kernel: &str,
        tg: u32,
        rows_per_tg: u32,
    ...
    pub fn mha_decode_flash_f16kv_tcb(
        ...
        if n_kv_heads == 0 || n_heads % n_kv_heads != 0 {
            return Err(Error::Metal(format!(
                "mha_decode_flash_f16kv_tcb: n_heads ({n_heads}) must be a multiple of n_kv_heads ({n_kv_heads})"
            )));
        }
        let group_size = (n_heads / n_kv_heads) as u32;
        let scale = 1.0_f32 / (head_dim as f32).sqrt();
    ...
    pub fn kv_quant_int4_append_tcb(
        ...
        // tg_id.y ∈ {0,1}.
        tcb.dispatch_threads(
            "kv_quant_int4_append",
            (n_kv_heads as u32 * hd, 2, 1),
            (hd, 1, 1),
            |enc| {
    ...
    pub fn mha_decode_flash_int4kv_tcb(
        ...
        if n_kv_heads == 0 || n_heads % n_kv_heads != 0 {
            return Err(Error::Metal(format!(
                "mha_decode_flash_int4kv_tcb: n_heads ({n_heads}) must be a multiple of n_kv_heads ({n_kv_heads})"
            )));
        }
        let group_size = (n_heads / n_kv_heads) as u32;
        let scale = 1.0_f32 / (head_dim as f32).sqrt();
    ...
    pub fn mha_decode_flash_int4kv_pc_tcb(
        ...
        if n_kv_heads == 0 || n_heads % n_kv_heads != 0 {
            return Err(Error::Metal(format!(
                "mha_decode_flash_int4kv_pc_tcb: n_heads ({n_heads}) must be a multiple of n_kv_heads ({n_kv_heads})"
            )));
        }
        let group_size = (n_heads / n_kv_heads) as u32;
        let scale = 1.0_f32 / (head_dim as f32).sqrt();
    ...
    fn encode_batched_gemv_indexed_tcb(
        ...
        let q8_w4_opt_in = std::env::var_os("HAWKING_Q8_DOWN_W4")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false);
        let q8_w2_opt_in = std::env::var_os("HAWKING_Q8_DOWN_W2")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false);
    ...
    pub fn encode_moe_block_batched_indexed_tcb_with_scratch(
        ...
        if routes == 0 {
            return Err(Error::Kernel(
                "encode_moe_block_batched_indexed_tcb_with_scratch: no routes".into(),
            ));
        }
...
mod residual_serve_tests {
    ...
    fn residual_two_part_gemv_matches_decoded_sum() {
        ...
        for &(rows, cols, b1, b2) in &cases {
            ...
            println!(
                "[residual_serve] {rows}x{cols} base{b1}+res{b2}: max_abs={max_abs:.3e} max_rel={max_rel:.3e}"
            );
            assert!(
                max_rel <= REL_TOL,
                "{rows}x{cols} base{b1}+res{b2}: max_rel {max_rel:.3e} > {REL_TOL:.1e} (GPU two-part vs decoded-sum)"
            );
    ...
    fn residual_file_round_trip_two_part_serves_decoded_sum() {
        ...
        let gpu_base = TqPreparedGpu::from_strand_tensor(st_base)
            ...
            .upload_to_gpu(&ctx)
            .expect("upload base");
        let gpu_res = TqPreparedGpu::from_strand_tensor(st_res)
            .expect("prep res")
            .upload_to_gpu(&ctx)
            .expect("upload res");
        let x_buf = ctx.new_buffer_with_bytes(bytemuck::cast_slice::<f32, u8>(&x));
