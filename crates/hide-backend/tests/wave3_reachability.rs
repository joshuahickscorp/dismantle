...
async fn trace_b_side_chat_folds_a_bounded_typed_result_not_the_full_transcript() {
 ...
 assert_eq!( payload.get("evidence").and_then(|v| v.as_array()).unwrap().len(), evidence.len() );
    for e in &parent_after {
        assert!(!serde_json::to_string(&e.payload).unwrap().contains(CHILD_SECRET));
    }
    ...
    assert!(parent_graph.edges.iter().any(|e| e.parent == parent && e.child == child));
 ...
 assert_eq!( ui.get("evidence").and_then(|v| v.as_array()).unwrap().len(), evidence.len() );
    ...
    assert!(child_events .iter() .any(|e| serde_json::to_string(&e.payload).unwrap().contains(CHILD_SECRET)));
    assert!(!child_events.iter().any(|e| e.kind == "session.merge_summary"));
    let summary_hits = host
        .search_transcript(&TranscriptQuery::literal("selection").in_session(parent.clone()))
        .await
        .unwrap();
    assert!(summary_hits.iter().any(|h| h.role.as_deref() == Some("side_chat")));
}
...
async fn diagnostics_feed_publishes_real_nonzero_counts_for_planted_issues() {
    ...
    assert_eq!(by_file[0]["errors"].as_u64().unwrap() + by_file[0]["warnings"].as_u64().unwrap(), errors + warnings);
